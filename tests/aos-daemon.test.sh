#!/usr/bin/env bash
# tests/aos-daemon.test.sh - supervise-daemon classifiers, the owner-relevant
...
# aos-send swallow reporting, composer-pending ANSI parsing). The operator-visible
# inject flow lives in aos-afk-inject-e2e and aos-wake-daemon-lifecycle-e2e.
set -u
...
TMP_ROOT=$(fm_test_tmproot aos-daemon-tests)


test_daemon_state_root_uses_fm_home() {
  local dir home override out
  dir=$(make_supercase daemon-aos-home)
  home="$dir/maestro-home"
  override="$dir/override-state"
  mkdir -p "$home" "$override"

  out=$(AOS_HOME="$home" AOS_STATE_OVERRIDE='' _state_root)
  [ "$out" = "$home/state" ] || fail "daemon state root ignored AOS_HOME: $out"

  out=$(AOS_HOME="$home" AOS_STATE_OVERRIDE="$override" _state_root)
  [ "$out" = "$override" ] || fail "daemon state root ignored AOS_STATE_OVERRIDE: $out"

  pass "supervise daemon state root is scoped by AOS_HOME"
}
...
test_classify_routine_signal_self() {
  ...
  out=$(AOS_STATE_OVERRIDE="$state" classify_signal "$state/foo-x1.status" "$state")
...
test_classify_terminal_signal_escalates() {
  ...
  for kw in "done: PR https://x/y/pull/1" "needs-decision: pick A" "blocked: no perms" \
    ...
    printf 'working\n%s\n' "$kw" > "$state/t.status"
    out=$(AOS_STATE_OVERRIDE="$state" classify_signal "$state/t.status" "$state")
    case "$out" in escalate\|*) ;; *) fail "owner verb did not escalate ($kw): $out" ;; esac
  done
  pass "owner-relevant status verbs escalate"
}
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:aos-qux-w4" "$state"
  out=$(AOS_STATE_OVERRIDE="$state" classify_stale "sess:aos-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:aos-qux-w4")" | tr ':/.' '___')
...
test_stale_terminal_escalates() {
  ...
  out=$(AOS_STATE_OVERRIDE="$state" classify_stale "sess:aos-fin-t5" "$state")
...
test_housekeeping_persistent_stale_escalates() {
  ...
  state="$dir/state"
  fakebin="$dir/fakebin"
  win="sess:aos-pers-w5"
  pane="$dir/pane.txt"
  printf 'working\n' > "$state/pers-w5.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "pers-w5" | tr ':/.' '___')
  echo $(( $(date +%s) - 500 )) > "$state/.subsuper-stale-$key"
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_WINDOW="$win" AOS_FAKE_TMUX_CAPTURE="$pane" \
...
test_escalate_batches_into_one_digest() {
  ...
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_PANE_ALIVE=1 AOS_FAKE_TMUX_SENT="$sent" \
    AOS_FAKE_TMUX_CAPTURE="$capture" AOS_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
...
test_escalate_batch_age_uses_first_append() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_PANE_ALIVE=1 AOS_FAKE_TMUX_SENT="$sent" \
    AOS_FAKE_TMUX_CAPTURE="$capture" AOS_ESCALATE_BATCH_SECS=90 AOS_HOUSEKEEPING_TICK=0 \
    housekeeping "$state"
  grep -F 'event A: done: PR 1 | event B: done: PR 2' "$sent" >/dev/null \
...
test_handle_wake_routes_self_and_escalate() {
  ...
  AOS_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "routine signal was escalated by handle_wake"
  printf 'done: PR 1\n' > "$state/h-done.status"
  AOS_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-done.status" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "owner signal was not buffered by handle_wake"
  pass "handle_wake routes routine->self and owner->escalate"
}
...
test_inject_skip_forces_self() {
  ...
  state="$dir/state"
  printf 'done: PR 1\n' > "$state/s1.status"
  AOS_STATE_OVERRIDE="$state" AOS_INJECT_SKIP="signal" handle_wake "signal: $state/s1.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "INJECT_SKIP=signal did not force self-handle"
  pass "INJECT_SKIP forces self-handle, bypassing owner-relevant classification"
}
...
test_is_wake_reason_distinguishes_status_stdout() {
  ...
  is_wake_reason "stale: s:aos-x" || fail "stale: not recognized as wake"
...
test_terminal_stale_escalate_leaves_no_marker() {
  ...
  echo $(( $(date +%s) - 500 )) > "$state/.subsuper-stale-$key"
  AOS_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  AOS_STATE_OVERRIDE="$state" handle_wake "signal: $state/sig-t8.status" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "owner signal was not escalated"
  key=$(printf '%s' "sig-t8" | tr ':/.' '___')
  [ "$(cat "$state/.subsuper-seen-status-$key" 2>/dev/null || true)" = "done: PR https://x/y/pull/8" ] \
    || fail "owner signal escalate did not write the seen-status marker"
  : > "$state/.subsuper-escalations"
  rm -f "$state/.subsuper-last-scan"
  AOS_STATE_OVERRIDE="$state" housekeeping "$state"
  [ ! -s "$state/.subsuper-escalations" ] || fail "catch-all scan re-fired an already-escalated signal"
  pass "owner signal escalate marks seen so the catch-all scan does not re-fire"
}
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" AOS_FAKE_TMUX_PANE_ALIVE=1 AOS_FAKE_TMUX_SENT="$sent" \
    AOS_FAKE_TMUX_CAPTURE="$capture" AOS_ESCALATE_BATCH_SECS=0 escalate_flush "$state"; then
...
test_busy_guard_defers_when_supervisor_busy() {
  ...
  # pane shows a busy signature (maestro mid-turn)
...
test_marker_detection() {
  ...
  message_is_injection "${AOS_INJECT_MARK}Supervisor escalate: done" \
...
test_afk_turn_exemption() {
  ...
  # a non-/afk skill invocation DOES exit (the owner is actively working)
...
test_should_exit_afk_when_afk_inactive() {
  ...
  should_exit_afk "$state" "${AOS_INJECT_MARK}test" \
...
test_strip_injection_marker() {
  local stripped
  stripped=$(strip_injection_marker "${AOS_INJECT_MARK}Supervisor escalate: done")
  ...
  # Only marker → empty.
  stripped=$(strip_injection_marker "$AOS_INJECT_MARK")
...
test_pane_input_pending_detects_partial_input() {
  ...
  printf 'line one\nline two\nhuman draft text\n' > "$capture"
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_CAPTURE="$capture" AOS_FAKE_TMUX_CURSOR_Y=2 \
...
test_pane_input_pending_honors_idle_override_after_border_strip() {
  ...
  capture="$dir/pane.txt"
  printf '│ custom idle> │\n' > "$capture"
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_CAPTURE="$capture" AOS_FAKE_TMUX_CURSOR_Y=0 \
    AOS_COMPOSER_IDLE_RE='^custom idle>$' pane_input_pending "fakepane" \
    && fail "AOS_COMPOSER_IDLE_RE was not applied after border stripping"
  pass "pane_input_pending honors AOS_COMPOSER_IDLE_RE after border stripping"
}
...
test_classify_signal_dedup_against_scan() {
  ...
  out=$(AOS_STATE_OVERRIDE="$state" classify_signal "$state/dup-s9.status" "$state")
...
test_classify_stale_dedup_against_signal() {
  ...
  out=$(AOS_STATE_OVERRIDE="$state" classify_stale "sess:aos-dup-s10" "$state")
...
test_pane_input_pending_bordered_idle_not_pending() {
  ...
  for line in \
    ...
    "│                                              │"; do
    printf '%s\n' "$line" > "$capture"
    if PATH="$fakebin:$PATH" AOS_FAKE_TMUX_CAPTURE="$capture" AOS_FAKE_TMUX_CURSOR_Y=0 \
...
test_pane_input_pending_bordered_with_text_is_pending() {
  ...
  # still read as pending so the daemon defers (and the owner-return race is
  # still protected).
  local dir state fakebin capture
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" AOS_FAKE_COMPOSER="$dir/composer" AOS_FAKE_SENT="$sent" \
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, aos-send exits non-zero —
  ...
  touch "$dir/.swallow"
  verdict=$(PATH="$fakebin:$PATH" AOS_FAKE_COMPOSER="$dir/composer" AOS_FAKE_SENT="$sent" \
    AOS_FAKE_SWALLOW="$dir/.swallow" AOS_FAKE_PERSIST_SWALLOW=1 \
    fm_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AOS_FAKE_COMPOSER="$dir/composer" AOS_FAKE_SENT="$sent" \
    AOS_FAKE_SWALLOW="$dir/.swallow" AOS_FAKE_PERSIST_SWALLOW=1 AOS_INJECT_CONFIRM_SLEEP=0.05 \
    AOS_ESCALATE_BATCH_SECS=99999 AOS_MAX_DEFER_SECS=60 housekeeping "$state"
...
test_max_defer_flushes_empty_idle_pane() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AOS_FAKE_COMPOSER="$dir/composer" AOS_FAKE_SENT="$sent" \
    AOS_ESCALATE_BATCH_SECS=99999 AOS_MAX_DEFER_SECS=60 AOS_INJECT_CONFIRM_SLEEP=0.05 \
    housekeeping "$state"
...
test_normal_flush_clears_stale_wedge_marker() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AOS_FAKE_COMPOSER="$dir/composer" AOS_FAKE_SENT="$sent" \
    AOS_INJECT_CONFIRM_SLEEP=0.05 escalate_flush "$state" \
...
test_below_max_defer_does_nothing() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_PANE_ALIVE=1 AOS_FAKE_TMUX_SENT="$sent" \
    AOS_FAKE_TMUX_CAPTURE="$capture" AOS_FAKE_TMUX_CURSOR_Y=0 \
    AOS_ESCALATE_BATCH_SECS=99999 AOS_MAX_DEFER_SECS=300 housekeeping "$state"
...
test_fm_send_exits_nonzero_on_confirmed_swallow() {
  # aos-send.sh must exit NON-ZERO when a steer's Enter is positively swallowed
  # (text left in the composer), so maestro learns the instruction did not land
  ...
  # Clean submit -> exit 0.
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$dir/state" AOS_FAKE_COMPOSER="$dir/composer" \
    AOS_SEND_SLEEP=0.05 "$ROOT/bin/aos-send.sh" sess:win 'route this work' >/dev/null 2>"$err" \
    || fail "aos-send exited non-zero on a clean submit: $(cat "$err")"
  # Persistent swallow -> exit non-zero with a clear message.
  printf '│ > │\n' > "$dir/composer"
  touch "$dir/.swallow"
  if PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$dir/state" AOS_FAKE_COMPOSER="$dir/composer" \
    AOS_FAKE_SWALLOW="$dir/.swallow" AOS_FAKE_PERSIST_SWALLOW=1 AOS_SEND_SLEEP=0.05 \
    "$ROOT/bin/aos-send.sh" sess:win 'fix findings 1 and 3, skip 2' >/dev/null 2>"$err"; then
    fail "aos-send exited zero despite a swallowed Enter (silent unsubmitted instruction)"
  fi
  grep -F 'not submitted' "$err" >/dev/null || fail "aos-send did not explain the swallowed submit: $(cat "$err")"
  pass "aos-send exits non-zero on a confirmed swallow, zero on a clean submit"
}
...
test_fm_send_exits_nonzero_on_initial_send_failure() {
  ...
  fakebin="$dir/fakebin"; err="$dir/send.err"
  if PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$dir/state" AOS_FAKE_COMPOSER="$dir/composer" \
    AOS_FAKE_SEND_FAIL=1 AOS_SEND_SLEEP=0.05 \
    "$ROOT/bin/aos-send.sh" sess:win 'route this work' >/dev/null 2>"$err"; then
    fail "aos-send exited zero despite initial tmux send-keys failure"
  fi
  grep -F 'text not sent' "$err" >/dev/null || fail "aos-send did not explain initial send failure: $(cat "$err")"
  pass "aos-send exits non-zero when initial text send fails"
}
