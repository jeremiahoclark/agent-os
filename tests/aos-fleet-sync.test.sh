#!/usr/bin/env bash
# Behavior tests for aos-fleet-sync.sh drift handling.
#
# aos-fleet-sync fast-forwards a clone that is cleanly on its default branch. This
...
TMP_ROOT=$(fm_test_tmproot aos-fleet-sync-tests)
HOME_N=0

# --- fixtures ---------------------------------------------------------------

# new_home: fresh isolated AOS_HOME with an empty projects/ dir. Each test gets its
...
run_sync() {
  local home=$1
  shift
  AOS_HOME="$home" AOS_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/aos-fleet-sync.sh" "$@" 2>/dev/null
}
...
test_bootstrap_relays_recovered_and_stuck() {
  ...
  # Full bootstrap: no state/ dir -> secondary_subagent sync no-ops; no .env -> X mode off.
  # We only assert the fleet-sync relay lines; other detect lines are irrelevant.
  out=$(AOS_HOME="$home" AOS_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/aos-bootstrap.sh" 2>/dev/null)
