#!/usr/bin/env bash
# tests/aos_secondary_subagent_lifecycle_e2e.test.sh - the happy-path secondary_subagent operator
...
# lease-specific paths live in aos_secondary_subagent_safety.test.sh.
#
...
#   - spawn meta records kind=secondary_subagent, home=, and the project list; launch runs
#     in the subhome with the persistent charter and cleared operational overrides
#   - a bare `aos-<id>` send targets the window recorded in THIS home's meta
...
TMP_ROOT=$(fm_test_tmproot aos_secondary_subagent_lifecycle)
...
EOF
  ...
  AOS_SECONDARY_SUBAGENT_SCOPE='customer onboarding from brief' \
    scaffold_secondary_subagent_charter "$HOME_DIR" design 'customer onboarding charter' alpha beta gamma \
    || fail "filled secondary_subagent charter scaffold failed"
}

phase_seed() {
  local out
  out=$(PATH="$FAKEBIN:$PATH" AOS_HOME="$HOME_DIR" \
    "$ROOT/bin/aos-home-seed.sh" design "$SUB" alpha beta gamma) \
    || fail "seed failed"
  SUB_ABS=$(cd "$SUB" && pwd -P)

  assert_contains "$out" "home=$SUB_ABS" "seed did not report the subhome"
  assert_present "$SUB/.aos_secondary_subagent_home" "seed did not mark the subhome"
  ...
  assert_grep '- design - customer onboarding charter' "$HOME_DIR/data/secondary_subagents.md" "registry summary not from the charter"
  assert_grep 'scope: customer onboarding from brief' "$HOME_DIR/data/secondary_subagents.md" "registry scope not from the filled brief"
  assert_grep 'projects: alpha, beta, gamma' "$HOME_DIR/data/secondary_subagents.md" "registry did not record the project list"
  assert_no_grep 'owns:' "$HOME_DIR/data/secondary_subagents.md" "registry used the legacy owns field"

  # Delivery modes preserved in the subhome registry; validation passes.
  [ "$(AOS_HOME="$SUB" "$ROOT/bin/aos-project-mode.sh" alpha)" = "direct-PR on" ] \
    || fail "alpha delivery mode not preserved in the subhome"
  [ "$(AOS_HOME="$SUB" "$ROOT/bin/aos-project-mode.sh" beta)" = "direct-PR off" ] \
    || fail "beta delivery mode not preserved in the subhome"
  AOS_HOME="$HOME_DIR" "$ROOT/bin/aos-home-seed.sh" validate >/dev/null || fail "registry validation failed after seed"
...
phase_spawn() {
  : > "$LOG"
  PATH="$FAKEBIN:$PATH" AOS_HOME="$HOME_DIR" AOS_CONFIG_OVERRIDE="$HOME_DIR/parent-config" \
    AOS_FAKE_TMUX_LOG="$LOG" AOS_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/aos-spawn.sh" design "$SUB" codex --secondary_subagent >/dev/null \
    || fail "secondary_subagent spawn failed"

  local meta="$HOME_DIR/state/design.meta"
  assert_grep 'kind=secondary_subagent' "$meta" "spawn meta did not record kind=secondary_subagent"
  ...
  assert_grep "AOS_HOME='$SUB_ABS'" "$LOG" "secondary_subagent launch did not set AOS_HOME to the subhome"
  assert_grep 'AOS_ROOT_OVERRIDE= AOS_STATE_OVERRIDE= AOS_DATA_OVERRIDE= AOS_PROJECTS_OVERRIDE=' "$LOG" "launch did not clear operational overrides"
  assert_grep 'AOS_CONFIG_OVERRIDE=' "$LOG" "launch did not clear the config override"
  assert_grep "$SUB_ABS/data/charter.md" "$LOG" "launch did not use the persistent charter"
  assert_no_grep 'notify=' "$LOG" "secondary_subagent codex launch included the parent turn-end notify hook"
  assert_no_grep 'turn-ended' "$LOG" "secondary_subagent codex launch referenced a parent turn-ended signal"
  assert_no_grep 'treehouse get' "$LOG" "secondary_subagent spawn ran a project treehouse get"
  pass "spawn: launches in the subhome with persistent charter, records routing meta"
}

phase_send() {
  : > "$LOG"
  # The meta window (maestro:aos-design) must win over a foreign same-named
  # window returned by list-windows.
  PATH="$FAKEBIN:$PATH" AOS_HOME="$HOME_DIR" AOS_FAKE_TMUX_WINDOW="other-session:aos-design" \
    AOS_FAKE_TMUX_LOG="$LOG" AOS_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/aos-send.sh" aos-design 'route this work' >/dev/null 2>&1 \
    || fail "aos-send failed for a bare maestro window with home metadata"
  # design is a kind=secondary_subagent target, so the request is prefixed with the
  # from-maestro marker (bin/aos-marker-lib.sh): the send targets the meta window
  # AND carries the marker label, and the original payload still follows it.
  assert_grep 'send-keys -t maestro:aos-design -l [aos-from-maestro]' "$LOG" "send did not use the window recorded in this home's meta, or did not mark the secondary_subagent request"
  assert_grep 'route this work' "$LOG" "the original request text did not survive the marker"
  assert_no_grep 'send-keys -t other-session:aos-design' "$LOG" "send targeted a foreign same-named window"
  pass "send: a bare aos-<id> secondary_subagent routes to the meta window with the from-maestro marker"
}
...
EOF
  local out before
  out=$(AOS_HOME="$HOME_DIR" "$ROOT/bin/aos-backlog-handoff.sh" design feat-x feat-y) \
  ...
  AOS_HOME="$HOME_DIR" "$ROOT/bin/aos-backlog-handoff.sh" design feat-x feat-y >/dev/null 2>&1 \
...
phase_recovery() {
  ...
  PATH="$FAKEBIN:$PATH" AOS_HOME="$HOME_DIR" AOS_FAKE_TMUX_LOG="$LOG" AOS_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/aos-spawn.sh" design "echo relaunch" --secondary_subagent >/dev/null 2>&1 \
  ...
  assert_grep 'window=maestro:aos-design' "$meta" "respawn did not reconstruct the direct-report window"
...
phase_teardown() {
  : > "$LOG"
  PATH="$FAKEBIN:$PATH" AOS_HOME="$HOME_DIR" AOS_FAKE_TMUX_LOG="$LOG" AOS_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/aos-teardown.sh" design >/dev/null 2>&1 \
    || fail "teardown failed for the empty secondary_subagent home"
  assert_absent "$SUB" "teardown did not remove the retired secondary_subagent home"
  assert_absent "$HOME_DIR/state/design.meta" "teardown did not clear the parent meta"
  assert_no_grep '- design ' "$HOME_DIR/data/secondary_subagents.md" "teardown did not remove the registry route"
