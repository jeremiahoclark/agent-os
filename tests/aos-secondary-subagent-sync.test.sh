#!/usr/bin/env bash
# Tests for the local-HEAD secondary_subagent sync: every secondary_subagent home tracks the
# PRIMARY maestro checkout's current default-branch commit by a purely LOCAL
# fast-forward (no origin fetch). Two hook points drive it - bin/aos-spawn.sh
# (before launching a secondary_subagent) and bin/aos-bootstrap.sh (a startup sweep of
# every live secondary_subagent home) - and both share the ff machinery in
# bin/aos-ff-lib.sh.
#
...
#   - The bootstrap sweep fast-forwards every live secondary_subagent home and reports a
#     nudge (NUDGE_SECONDARY_SUBAGENTS:) ONLY for a running secondary_subagent whose instruction
#     surface actually changed; an already-current or readme-only home is never
#     nudged, a skipped home is reported as SECONDARY_SUBAGENT_SYNC:, and a home with no
#     live metadata is never swept.
#   - Spawning a secondary_subagent fast-forwards its worktree to the primary's HEAD
...
TMP_ROOT=$(fm_test_tmproot aos_secondary_subagent_sync)

# --- world builders --------------------------------------------------------

# new_world <name>: a PRIMARY maestro repo on `main` with one commit (the
...
new_world() {
  ...
  # Fresh watcher beacon keeps aos-guard quiet for the spawn path.
  touch "$w/home/state/.last-watcher-beat"

  git init -q -b main "$w/main"
  # Mirror the real repo: the gitignored operational dirs never dirty a worktree,
  # so a secondary_subagent home's data/state/projects can never block its fast-forward.
...
# add_sm_worktree <w> <id> <commit>: a secondary_subagent home as a DETACHED worktree of
# the primary at <commit>, plus its seed marker and a LIVE kind=secondary_subagent meta
...
# status line. Uses allow_detached=yes, ignore_seed_marker=yes (the secondary_subagent
# home contract).
FF_OUT=""
run_ff() {
  local dir=$1 base=$2 outfile="$TMP_ROOT/ff.out"
  ff_target "$dir" "secondary_subagent sm" "$base" yes yes >"$outfile" 2>&1
  FF_OUT=$(cat "$outfile")
}
...
test_ff_updated() {
  ...
  assert_contains "$FF_OUT" "secondary_subagent sm: updated " "updated home prints an advance line"
...
test_ff_current() {
  ...
  assert_contains "$FF_OUT" "secondary_subagent sm: already current" "current home reports already current"
...
test_ff_dirty() {
  ...
  assert_contains "$FF_OUT" "secondary_subagent sm: skipped: dirty working tree" "dirty home is skipped"
...
test_ff_diverged() {
  ...
  [ "$FF_STATUS" = skipped ] || fail "FF_STATUS: expected skipped, got '$FF_STATUS'"
  assert_contains "$FF_OUT" "secondary_subagent sm: skipped: diverged from $base" "diverged home is skipped"
...
# A secondary_subagent home carrying its own in-flight work sits on a named feature
...
test_ff_inflight_feature_branch() {
  ...
  [ "$FF_STATUS" = skipped ] || fail "FF_STATUS: expected skipped, got '$FF_STATUS'"
  assert_contains "$FF_OUT" "secondary_subagent sm: skipped: on feature/wip, expected main" \
...
test_sweep_nudge_requires_instruction_change() {
  ...
  AOS_ROOT="$w/main" AOS_HOME="$w/home"
  FF_NUDGE_WINDOWS=""
  FF_SEEN_HOMES=""
  sweep_live_secondary_subagent_metas "$w/home/state" "$base" yes >/dev/null
...
test_bootstrap_sweep_nudges_only_instruction_change() {
  ...
  printf 'sm-nonlive\n' > "$w/sm-nonlive/.aos_secondary_subagent_home"

  fakebin=$(make_fake_toolchain "$w")
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$w/home" AOS_ROOT_OVERRIDE="$w/main" \
    "$ROOT/bin/aos-bootstrap.sh" 2>/dev/null)

  nudge_line=$(printf '%s\n' "$out" | grep '^NUDGE_SECONDARY_SUBAGENTS:' || true)
  [ -n "$nudge_line" ] || fail "no NUDGE_SECONDARY_SUBAGENTS line emitted (got: $out)"
  assert_contains "$nudge_line" "maestro:aos-sm-instr" "instruction-changed running secondary_subagent is nudged"
  assert_not_contains "$nudge_line" "sm-readme" "readme-only advance is not nudged"
  assert_not_contains "$nudge_line" "sm-current" "already-current secondary_subagent is not nudged"
  ...
  pass "T8 bootstrap sweeps live homes, nudges only the running real-instruction-change secondary_subagent"
}

# --- T9: bootstrap surfaces a skipped dirty live secondary_subagent home --------------
test_bootstrap_sweep_surfaces_skipped_home() {
  ...
  skip_line=$(printf '%s\n' "$out" | grep '^SECONDARY_SUBAGENT_SYNC: secondary_subagent sm-dirty: skipped:' || true)
  [ -n "$skip_line" ] || fail "no SECONDARY_SUBAGENT_SYNC skip line emitted (got: $out)"
  ...
  pass "T9 bootstrap surfaces a skipped dirty live secondary_subagent home"
}

# --- T10: spawning a secondary_subagent fast-forwards its worktree before launch ------
test_spawn_fast_forwards_before_launch() {
  local w c1 c2 fakebin
  w=$(new_world spawn-ff)
  c1=$(head_of "$w/main")
  git -C "$w/main" worktree add -q --detach "$w/sm" "$c1"
  printf 'sm\n' > "$w/sm/.aos_secondary_subagent_home"
  mkdir -p "$w/sm/data"
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    AOS_ROOT_OVERRIDE="$w/main" AOS_HOME="$w/home" \
    AOS_STATE_OVERRIDE="$w/home/state" AOS_DATA_OVERRIDE="$w/home/data" \
    AOS_PROJECTS_OVERRIDE="$w/home/projects" AOS_CONFIG_OVERRIDE="$w/home/config" \
    AOS_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/aos-spawn.sh" sm "$w/sm" codex --secondary_subagent >/dev/null 2>&1 || true

  [ "$(head_of "$w/sm")" = "$c2" ] \
    || fail "spawn did not fast-forward the secondary_subagent worktree to the primary's HEAD"
  pass "T10 spawn fast-forwards a secondary_subagent worktree to the primary's local HEAD before launch"
}
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    AOS_PROJECTS_OVERRIDE="$w/home/projects" AOS_CONFIG_OVERRIDE="$w/home/config" \
    AOS_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/aos-spawn.sh" sm "$w/sm" codex --secondary_subagent >/dev/null 2>"$err" || true

  assert_contains "$(cat "$err")" \
    "warning: secondary_subagent sm sync skipped before launch: dirty working tree" \
...
echo "# all aos_secondary_subagent_sync tests passed"
