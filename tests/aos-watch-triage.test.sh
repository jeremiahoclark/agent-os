#!/usr/bin/env bash
# tests/aos-watch-triage.test.sh - the always-on wake triage built into
# bin/aos-watch.sh and the shared classifier (bin/aos-classify-lib.sh). The watcher
# now absorbs the benign majority of wakes in bash and exits ONLY on an actionable
# wake, so maestro's LLM re-arms once per actionable event instead of once per
# wake. These tests cover the classifier predicates as pure functions, then drive
# a real aos-watch.sh subprocess to assert the behavioral contract:
...
#
# Daemon-side classification/injection lives in aos-daemon.test.sh; watcher/lock
# liveness in aos-watcher-lock.test.sh; the durable-queue safety matrix in
# aos-wake-queue.test.sh.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"
# shellcheck source=bin/aos-classify-lib.sh
. "$ROOT/bin/aos-classify-lib.sh"

WATCH="$ROOT/bin/aos-watch.sh"
DRAIN="$ROOT/bin/aos-wake-drain.sh"

TMP_ROOT=$(fm_test_tmproot aos-watch-triage-tests)

# Common watcher knobs: tight poll/grace, no check or heartbeat cadence unless a
# test overrides them, so a test only exercises the path it targets. AOS_SUBAGENT_STATE_BIN
# points at the case's hermetic fake aos-subagent-state.sh (installed by make_case) so the
# absorb-only-when-provably-working triage reads a canned verdict; a test fixes that
# verdict via AOS_FAKE_CREW_STATE in its environment before calling watch_bg.
watch_bg() {  # <state> <fakebin> <out> [extra env assignments...]
  local state=$1 fakebin=$2 out=$3
  shift 3
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_SUBAGENT_STATE_BIN="$fakebin/aos-subagent-state.sh" \
    AOS_POLL=1 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$@" "$WATCH" > "$out" &
}
...
# fallback (which writes a partial filesystem dump on Linux; see aos-watch.sh).
file_mtime() {
  if [ "$(uname)" = Darwin ]; then stat -f %m "$1" 2>/dev/null; else stat -c %Y "$1" 2>/dev/null; fi
}

# Signature a primed .seen-* marker must hold so the per-poll signal scan does not
# fire on a pre-existing status (mirrors aos-watch.sh's stat_sig exactly).
seen_sig() {
...
# --- pure classifier predicates (aos-classify-lib.sh) ------------------------
...
test_signal_reason_is_actionable_classifier() {
  ...
  signal_reason_is_actionable "$state/b.status" || fail "owner-relevant signal classified benign"
  : > "$state/c.turn-ended"
  signal_reason_is_actionable "$state/c.turn-ended" && fail "a bare turn-ended marker classified actionable"
  # Coalesced batch: one benign + one owner-relevant -> actionable.
  signal_reason_is_actionable "$state/a.status" "$state/b.status" || fail "coalesced benign+actionable not actionable"
  pass "signal_reason_is_actionable: benign absorbed, owner verbs and coalesced batches surfaced"
}
...
test_stale_is_terminal_classifier() {
  ...
  stale_is_terminal "sess:aos-term" "$state" || fail "terminal stale status not classified terminal"
  printf 'working: compiling\n' > "$state/nonterm.status"
  stale_is_terminal "sess:aos-nonterm" "$state" && fail "non-terminal stale classified terminal"
  stale_is_terminal "sess:aos-missing" "$state" && fail "stale with no status classified terminal"
  pass "stale_is_terminal: terminal status surfaces, non-terminal and no-status are benign"
}

test_scan_owner_relevant_statuses_classifier() {
  local dir state out
  dir=$(make_case classify-scan); state="$dir/state"
  printf 'working: a\n' > "$state/one.status"
  printf 'blocked: no perms\n' > "$state/two.status"
  printf 'done: PR https://x/y/pull/1\n' > "$state/three.status"
  out=$(scan_owner_relevant_statuses "$state")
  ...
  pass "scan_owner_relevant_statuses lists only owner-relevant statuses"
}
...
test_classifier_primitives() {
  ...
  status_is_owner_relevant "done: b" || fail "done: not recognized as owner-relevant"
  status_is_owner_relevant "working: b" && fail "working: wrongly recognized as owner-relevant"
  [ "$(window_to_task "sess:aos-fix-login-k3")" = "fix-login-k3" ] || fail "window_to_task did not strip session+aos- prefix"
  AOS_OWNER_RE='custom-verb:' status_is_owner_relevant "custom-verb: x" || fail "AOS_OWNER_RE override not honored"
  AOS_OWNER_RE='custom-verb:' status_is_owner_relevant "done: x" && fail "AOS_OWNER_RE override did not replace the default verb set"
  pass "classifier primitives: last line, owner-relevance, window->task, AOS_OWNER_RE override"
}

# subagent_is_provably_working: the absorb-only-when-provably-working predicate. It is
# benign (absorb) ONLY when aos-subagent-state.sh reports the crew as working from an
...
# fake aos-subagent-state.sh (AOS_SUBAGENT_STATE_BIN) returns a canned verdict per case.
test_subagent_is_provably_working_classifier() {
  local dir fakebin
  ...
  export AOS_SUBAGENT_STATE_BIN="$fakebin/aos-subagent-state.sh"
  export AOS_FAKE_CREW_STATE
  AOS_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
  subagent_is_provably_working a || fail "active run-step not treated as provably working"
  AOS_FAKE_CREW_STATE='state: working · source: pane · harness busy'
  subagent_is_provably_working a || fail "busy pane not treated as provably working"
  AOS_FAKE_CREW_STATE='state: working · source: status-log · working: compiling'
  ! subagent_is_provably_working a || fail "stale status-log working: treated as provably working"
  AOS_FAKE_CREW_STATE='state: done · source: run-step · checks green'
  ! subagent_is_provably_working a || fail "finished run treated as provably working"
  AOS_FAKE_CREW_STATE='state: parked · source: run-step · parked at review'
  ! subagent_is_provably_working a || fail "parked run treated as provably working"
  AOS_FAKE_CREW_STATE='state: failed · source: run-step · run failed'
  ! subagent_is_provably_working a || fail "failed run treated as provably working"
  AOS_FAKE_CREW_STATE='state: unknown · source: none · worktree gone'
  ! subagent_is_provably_working a || fail "unknown crew treated as provably working"
  AOS_FAKE_CREW_STATE='state: working · source: run-step · x'
  ! subagent_is_provably_working "" || fail "empty id treated as provably working"
  unset AOS_FAKE_CREW_STATE
  pass "subagent_is_provably_working: only working+run-step/pane is provable; idle/finished/parked/failed/unknown surface"
}
...
test_signal_crew_provably_working_classifier() {
  ...
  export AOS_SUBAGENT_STATE_BIN="$fakebin/aos-subagent-state.sh"
  export AOS_FAKE_CREW_STATE_a='state: working · source: run-step · running'
  export AOS_FAKE_CREW_STATE_b='state: done · source: run-step · run passed'
  ...
  unset AOS_FAKE_CREW_STATE_a AOS_FAKE_CREW_STATE_b
...
test_provably_working_signal_absorbed() {
  ...
  export AOS_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
...
test_turn_ended_provably_working_absorbed() {
  ...
  export AOS_FAKE_CREW_STATE='state: working · source: pane · harness busy'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  if ! wait_live "$pid" 30; then
...
# reports its final turn-end with no owner-relevant status and no running
...
test_turn_ended_not_working_surfaced() {
  ...
  export AOS_FAKE_CREW_STATE='state: unknown · source: none · no current-state source available'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced turn-end failed"
...
test_working_note_not_working_surfaced() {
  ...
  status_file="$state/task.status"
  printf 'working: compiling step 2\n' > "$status_file"
  # A non-no-mistakes crew (no run) whose pane went idle: aos_subagent_state falls back
  ...
  export AOS_FAKE_CREW_STATE='state: working · source: status-log · working: compiling step 2'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced working: note failed"
...
test_actionable_signal_surfaced() {
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the actionable signal failed"
  ...
  pass "owner-relevant signal is surfaced (queue + exit) and marked surfaced"
}
...
test_terminal_stale_surfaced() {
  ...
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_WINDOW="$window" AOS_FAKE_TMUX_CAPTURE="$capture_file" \
    AOS_STATE_OVERRIDE="$state" AOS_POLL=1 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the terminal stale failed"
...
test_nonterminal_stale_provably_working_absorbed_then_escalated() {
  ...
  export AOS_FAKE_CREW_STATE='state: working · source: run-step · ci running'

  # Phase A: a high escalation threshold means the first sighting is absorbed.
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_WINDOW="$window" AOS_FAKE_TMUX_CAPTURE="$capture_file" \
    AOS_STATE_OVERRIDE="$state" AOS_SUBAGENT_STATE_BIN="$fakebin/aos-subagent-state.sh" AOS_STALE_ESCALATE_SECS=999 AOS_POLL=1 AOS_SIGNAL_GRACE=1 \
  ...
  : > "$out"
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_WINDOW="$window" AOS_FAKE_TMUX_CAPTURE="$capture_file" \
    AOS_STATE_OVERRIDE="$state" AOS_SUBAGENT_STATE_BIN="$fakebin/aos-subagent-state.sh" AOS_STALE_ESCALATE_SECS=240 AOS_POLL=1 AOS_SIGNAL_GRACE=1 \
    AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the wedge escalation failed"
...
test_nonterminal_stale_not_working_surfaced() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:aos-stopped"
  printf 'idle prompt, finished' > "$capture_file"
  printf 'window=%s\nkind=ship\n' "$window" > "$state/stopped.meta"
  # Non-terminal status (the crew never wrote an owner-relevant verb), .seen-*
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the immediate stale failed"
...
test_nonterminal_stale_repairs_missing_or_corrupt_timer() {
  ...
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_WINDOW="$window" AOS_FAKE_TMUX_CAPTURE="$capture_file" \
    AOS_STATE_OVERRIDE="$state" AOS_STALE_ESCALATE_SECS=999 AOS_POLL=1 AOS_SIGNAL_GRACE=1 \
    AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
SH
  ...
  export AOS_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_SUBAGENT_STATE_BIN="$fakebin/aos-subagent-state.sh" AOS_POLL=1 AOS_SIGNAL_GRACE=1 \
    AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 AOS_WATCH_TRIAGE_LOG_MAX_BYTES=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
...
test_heartbeat_no_change_absorbed() {
  ...
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=1 AOS_SIGNAL_GRACE=1 \
    AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
  ...
  reap "$pid"
  pass "a heartbeat with no owner-relevant change is absorbed and backs off the cadence"
}
...
test_heartbeat_backstop_surfaces_unsurfaced_status() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"
  # A owner-relevant status whose .seen-* signature ALREADY matches (so the
  ...
  # fleet-scan backstop must catch it and wake maestro.
  printf 'done: PR https://example.test/pr/5\n' > "$state/miss.status"
  sig=$(seen_sig "$state/miss.status"); printf '%s' "$sig" > "$state/.seen-miss_status"
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=1 AOS_SIGNAL_GRACE=1 \
    AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=1 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 40 || fail "heartbeat backstop did not surface an unsurfaced owner-relevant status"
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the backstop heartbeat failed"
  grep "$(printf '\theartbeat\t')" "$drain_out" >/dev/null || fail "backstop heartbeat was not queued"
  pass "heartbeat backstop fail-safe surfaces an owner-relevant status the per-wake path missed"
}
...
test_beacon_stays_fresh_while_absorbing() {
  ...
  reap "$pid"
  pass "the liveness beacon stays fresh while the watcher absorbs benign wakes (aos-guard never false-alarms)"
}
...
test_afk_present_reverts_watcher_to_one_shot() {
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the afk-mode signal failed"
