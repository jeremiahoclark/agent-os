#!/usr/bin/env bash
# Behavior tests for aos-bootstrap.sh tool detection.
#
# Bootstrap prints one line per problem or capability fact and is silent when all
# is well. maestro consumes the exact 'MISSING: treehouse (install: ...)' and
...
TMP_ROOT=$(fm_test_tmproot aos-bootstrap-tests)

# A fake toolchain where every required tool is present and gh is authenticated.
# treehouse's `get --help` advertises --lease only when AOS_FAKE_TREEHOUSE_LEASE_HELP=1.
make_fake_toolchain() {
  local dir=$1 fakebin
...
if [ "${1:-}" = get ] && [ "${2:-}" = --help ]; then
  if [ "${AOS_FAKE_TREEHOUSE_LEASE_HELP:-}" = 1 ]; then
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${AOS_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.31.2 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
test_bootstrap_reporting() {
  ...
  while IFS='^' read -r label lease tasks mode expect notcontains; do
    ...
    out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$case_dir/home" AOS_ROOT_OVERRIDE="$case_dir/home" \
      AOS_FAKE_TREEHOUSE_LEASE_HELP="$lease" "$ROOT/bin/aos-bootstrap.sh")
    case "$mode" in
      empty)
