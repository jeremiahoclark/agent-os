#!/usr/bin/env bash
# tests/aos-afk-inject-e2e.test.sh - private-socket end-to-end test for the afk
...
#     A owner-relevant status must deliver exactly ONE sentinel-prefixed,
...
# private socket. The daemon points at a throwaway state dir (AOS_STATE_OVERRIDE)
# and the test pane (AOS_SUPERVISOR_TARGET). Nothing touches the live fleet.
#
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/aos-afk-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
LOG_FILE="$STATE_DIR/submitted.log"
: > "$LOG_FILE"

# Source the daemon to get AOS_INJECT_MARK, afk_enter, afk_exit.
...
TMUX_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/aos-shim.XXXXXX")
...
# Create a fake subagent window (the watcher lists aos-* windows for stale
# detection). The pane is an inert shell — it just needs to exist.
"$REAL_TMUX" -L "$SOCKET" new-window -d -n aos-fake-c1 -t supervisor

start_daemon() {
  PATH="$TMUX_SHIM_DIR:$PATH" \
  AOS_STATE_OVERRIDE="$STATE_DIR" \
  AOS_SUPERVISOR_TARGET="$SUPERVISOR_PANE" \
  AOS_ESCALATE_BATCH_SECS=0 \
  AOS_HOUSEKEEPING_TICK=1 \
  AOS_POLL=1 \
  AOS_SIGNAL_GRACE=1 \
  AOS_HEARTBEAT=999999 \
  AOS_CHECK_INTERVAL=999999 \
  AOS_INJECT_CONFIRM_SLEEP=0.3 \
  AOS_INJECT_CONFIRM_RETRIES=5 \
  AOS_STALE_ESCALATE_SECS=999999 \
...
test_scenario_a() {
  ...
  # owner returning and starting to type before afk has been cleared.
  "$REAL_TMUX" -L "$SOCKET" send-keys -t "$SUPERVISOR_PANE" -l "human draft text"
  sleep 0.5

  # Write an owner-relevant status to trigger a real escalation through the
  # real watcher child.
...
test_scenario_b() {
  ...
  # Write an owner-relevant status to trigger a real escalation.
...
# No human input, no swallowed Enter: an owner-relevant status must produce
...
test_scenario_c() {
  ...
  stop_daemon
  pass "Scenario C: a normal owner status injects exactly one clean single-line sentinel digest"
}
