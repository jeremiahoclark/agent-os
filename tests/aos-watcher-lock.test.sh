#!/usr/bin/env bash
# tests/aos-watcher-lock.test.sh - watcher singleton + lock-primitive races +
...
WATCH_ARM="$ROOT/bin/aos-watch-arm.sh"
DRAIN="$ROOT/bin/aos-wake-drain.sh"
LIB="$ROOT/bin/aos-wake-lib.sh"

TMP_ROOT=$(fm_test_tmproot aos-watcher-lock-tests)
...
test_singleton_start() {
  ...
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out1" &
  pid1=$!
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out2" &
  pid2=$!
  sleep 0.5
  live=0
...
test_stale_watch_lock_reclaimed() {
  ...
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  sleep 0.5
  live=0
...
test_live_stale_watch_lock_is_actionable() {
  ...
  status=0
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_GUARD_GRACE=1 AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out" 2> "$err" || status=$?
...
test_guard_warnings() {
  ...
  # AOS_ROOT_OVERRIDE points the worktree-tangle check at a non-git dir so it stays
  ...
  AOS_ROOT_OVERRIDE="$dir" AOS_STATE_OVERRIDE="$state" AOS_GUARD_GRACE=1 "$ROOT/bin/aos-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  ...
  grep -F 'bin/aos-watch-arm.sh' "$err" >/dev/null || fail "guard banner missing the fix command"
  ...
  # Non-git AOS_ROOT keeps the worktree-tangle check inert so "fresh watcher ->
  # total silence" stays a pure assertion about watcher state.
  AOS_ROOT_OVERRIDE="$dir" AOS_STATE_OVERRIDE="$state" AOS_GUARD_GRACE=300 "$ROOT/bin/aos-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
...
test_lock_steals_dead_pid_lock() {
  ...
  printf '%s\n' "$dead" > "$lockdir/pid"
  rc=0
  newpid=$(AOS_STATE_OVERRIDE="$state" bash -c '
    . "$1"
...
test_lock_live_steal_mutex_is_not_reclaimed() {
  ...
  out=$(AOS_LOCK_STALE_AFTER=0 AOS_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if fm_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s lockpid=%s stealpid=%s\n" "$rc" "${AOS_LOCK_HELD_PID:-}" "$(cat "$2/pid" 2>/dev/null || true)" "$(cat "$2.steal/pid" 2>/dev/null || true)"
  ' _ "$LIB" "$lockdir")
...
test_lock_does_not_steal_live_lock() {
  ...
  mkdir "$lockdir"
  printf '%s\n' "$live" > "$lockdir/pid"
  out=$(AOS_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if fm_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s\n" "$rc" "${AOS_LOCK_HELD_PID:-}"
  ' _ "$LIB" "$lockdir")
  ...
  case "$out" in
    *"held=$live"*) ;;
    *) fail "live holder pid not reported via AOS_LOCK_HELD_PID: $out" ;;
  esac
...
test_lock_paused_mid_acquire_claim_fails_during_steal() {
  ...
  out=$(AOS_LOCK_STALE_AFTER=0 AOS_STATE_OVERRIDE="$state" bash -c '
    ...
    steal_owner=${AOS_LOCK_OWNER_DIR:-}
...
test_watch_restart_rejects_reused_pid() {
  ...
  printf '%s\n' "$dir" > "$state/.watch.lock/aos-home"
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "stale watcher identity" > "$state/.watch.lock/pid-identity"
  PATH="$fakebin:$PATH" AOS_HOME="$dir" AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH_ARM" --restart > "$out" &
  pid=$!
...
test_watcher_self_evicts_on_lock_takeover() {
  ...
  fakebin="$dir/fakebin"
  out="$dir/watch.out"
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=1 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  i=0
  while [ "$i" -lt 50 ]; do
...
test_arm_reports_healthy_for_live_fresh_watcher() {
  ...
  status=0
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" "$WATCH_ARM" > "$armout" || status=$?
...
test_arm_starts_and_self_heals() {
  ...
  for row in clean dead-pid; do
    ...
    PATH="$fakebin:$PATH" AOS_HOME="$dir" AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    armpid=$!
    i=0
    while [ "$i" -lt 80 ]; do
...
test_arm_hup_cleans_child_and_temp_output() {
  ...
  fakebin="$dir/fakebin"
  armout="$dir/arm.out"
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
SH
  chmod +x "$check_file"
  rc=0
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_GUARD_GRACE=0 AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=0 AOS_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" || rc=$?
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after immediate arm wake failed"
...
test_arm_waits_for_peer_beacon_after_child_stands_down() {
  ...
  sleep 300 &
  peer=$!
  identity=$(AOS_STATE_OVERRIDE="$state" bash -c '. "$1"; fm_pid_identity "$2"' _ "$LIB" "$peer") || fail "could not identify peer pid"
  mkdir "$state/.watch.lock"
  ...
  beater=$!
  status=0
  PATH="$fakebin:$PATH" AOS_HOME="$dir" AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 AOS_ARM_CONFIRM_TIMEOUT=4 "$WATCH_ARM" > "$armout" || status=$?
...
test_arm_fails_loud_when_no_fresh_watcher_confirmable() {
  ...
  printf '%s\n' "$live" > "$state/.watch.lock/pid"
  touch -t 200001010000 "$state/.last-watcher-beat"
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=5 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 AOS_ARM_CONFIRM_TIMEOUT=3 "$WATCH_ARM" > "$armout" &
  armpid=$!
