#!/usr/bin/env bash
# Behavior tests for bin/aos-subagent-state.sh - the deterministic crew-current-state
# helper.
#
# The status file (state/<id>.status) is a best-effort append-only EVENT LOG, so
# `tail -1` of it reports the last event, not the current state. aos_subagent_state
...
CREW_STATE="$ROOT/bin/aos-subagent-state.sh"
TMP_ROOT=$(fm_test_tmproot aos_subagent_state)
...
# bare `axi` (the run list). Each returns the matching AOS_FAKE_AXI_* env text.
...
case "${1:-}" in
  status)
    shift
    if [ "${1:-}" = --run ]; then printf '%s\n' "${AOS_FAKE_AXI_STATUS_RUN:-}"
    else printf '%s\n' "${AOS_FAKE_AXI_STATUS:-}"; fi ;;
  '') printf '%s\n' "${AOS_FAKE_AXI_LIST:-}" ;;
esac
exit 0
SH
...
case "${1:-}" in
  display-message)
    [ "${AOS_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    printf '%%1\n' ;;
  capture-pane)
    [ "${AOS_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    if [ "${AOS_FAKE_BUSY:-0}" = 1 ]; then printf 'work in progress\nesc to interrupt\n'
...
# Run the helper for one case dir. AOS_FAKE_* env (run output, busy flag) are read
# from the caller's environment by the fakes above.
run_crew_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" AOS_STATE_OVERRIDE="$1/state" "$CREW_STATE" "$2"
}
...
reset_fakes() {
  ...
  AOS_FAKE_AXI_LIST=""
  AOS_FAKE_BUSY=0
  AOS_FAKE_TMUX_MISSING=0
  export AOS_FAKE_AXI_STATUS AOS_FAKE_AXI_STATUS_RUN AOS_FAKE_AXI_LIST AOS_FAKE_BUSY AOS_FAKE_TMUX_MISSING
}
...
test_active_run_is_authoritative() {
  ...
  aos_write_meta "$d/state/feat-a.meta" "window=fm:aos-feat-a" "worktree=$d/wt" "kind=ship"
  AOS_FAKE_AXI_STATUS="$(run_running fm/feat-a)"
...
test_stale_needs_decision_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-b.meta" "window=fm:aos-feat-b" "worktree=$d/wt" "kind=ship"
  printf 'working: started\nneeds-decision: pick A or B\n' > "$d/state/feat-b.status"
  AOS_FAKE_AXI_STATUS="$(run_fixing fm/feat-b)"
...
test_stale_blocked_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-bb.meta" "window=fm:aos-feat-bb" "worktree=$d/wt" "kind=ship"
  printf 'blocked: waiting on review answer\n' > "$d/state/feat-bb.status"
  AOS_FAKE_AXI_STATUS="$(run_running fm/feat-bb)"
...
test_genuine_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-c.meta" "window=fm:aos-feat-c" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-c.status"
  AOS_FAKE_AXI_STATUS="$(run_parked fm/feat-c)"
...
test_scalar_gate_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-cs.meta" "window=fm:aos-feat-cs" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cs.status"
  AOS_FAKE_AXI_STATUS="$(run_parked_scalar_gate_running fm/feat-cs)"
...
test_gate_block_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-cb.meta" "window=fm:aos-feat-cb" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cb.status"
  AOS_FAKE_AXI_STATUS="$(run_parked_in_gate_block fm/feat-cb)"
...
test_ci_ready_done_log_beats_monitoring_run() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-ci.meta" "window=fm:aos-feat-ci" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-ci.status"
  AOS_FAKE_AXI_STATUS="$(run_ci_monitoring fm/feat-ci)"
...
test_terminal_passed() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-d.meta" "window=fm:aos-feat-d" "worktree=$d/wt" "kind=ship"
  AOS_FAKE_AXI_STATUS="$(run_passed fm/feat-d)"
...
test_terminal_failed() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-e.meta" "window=fm:aos-feat-e" "worktree=$d/wt" "kind=ship"
  AOS_FAKE_AXI_STATUS="$(run_failed fm/feat-e)"
...
test_cross_branch_attribution_via_list() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-f.meta" "window=fm:aos-feat-f" "worktree=$d/wt" "kind=ship"
  # The repo-wide active/most-recent run belongs to a different crew's branch.
  AOS_FAKE_AXI_STATUS="$(run_running fm/other-crew)"
  AOS_FAKE_AXI_LIST="$(cat <<EOF
runs[2]{id,branch,status,head,pr}:
  "01OTHER",fm/other-crew,running,aa,""
  "01MINE",fm/feat-f,running,bb,""
EOF
)"
  AOS_FAKE_AXI_STATUS_RUN="$(run_running fm/feat-f)"
...
test_cross_branch_attribution_unquoted_run_list() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-fq.meta" "window=fm:aos-feat-fq" "worktree=$d/wt" "kind=ship"
  AOS_FAKE_AXI_STATUS="$(run_running fm/other-crew)"
  AOS_FAKE_AXI_LIST="$(cat <<EOF
runs[2]{id,branch,status,head,pr}:
  01OTHER, "fm/other-crew" ,running,aa,""
...
EOF
  ...
  AOS_FAKE_AXI_STATUS_RUN="$(run_running fm/feat-fq)"
...
test_other_branch_run_ignored() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-g.meta" "window=fm:aos-feat-g" "worktree=$d/wt" "kind=ship"
  printf 'done: implemented, ready to validate\n' > "$d/state/feat-g.status"
  AOS_FAKE_AXI_STATUS="$(run_running fm/some-other)"
  AOS_FAKE_AXI_LIST="$(cat <<EOF
...
test_no_run_busy_pane() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-h.meta" "window=fm:aos-feat-h" "worktree=$d/wt" "kind=ship"
...
test_no_run_idle_pane_uses_log() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-i.meta" "window=fm:aos-feat-i" "worktree=$d/wt" "kind=ship"
...
test_dead_window_ignores_stale_status_log() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-dead.meta" "window=fm:aos-feat-dead" "worktree=$d/wt" "kind=ship"
  printf 'done: old completion event\n' > "$d/state/feat-dead.status"
  AOS_FAKE_AXI_STATUS=""
  AOS_FAKE_AXI_LIST=""
  AOS_FAKE_TMUX_MISSING=1
...
test_dead_window_still_reports_terminal_run_step() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-dead-done.meta" "window=fm:aos-feat-dead-done" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/3 checks green\n' > "$d/state/feat-dead-done.status"
  AOS_FAKE_AXI_STATUS="$(run_passed fm/feat-dead-done)"
  AOS_FAKE_TMUX_MISSING=1   # the crew's window has closed
...
test_dead_window_still_reports_active_run_step() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/feat-dead-act.meta" "window=fm:aos-feat-dead-act" "worktree=$d/wt" "kind=ship"
  AOS_FAKE_AXI_STATUS="$(run_running fm/feat-dead-act)"
  AOS_FAKE_TMUX_MISSING=1
...
SH
  ...
  aos_write_meta "$d/state/feat-timeout.meta" "window=fm:aos-feat-timeout" "worktree=$d/wt" "kind=ship"
  AOS_FAKE_BUSY=1
  start=$SECONDS
  out=$(PATH="$d/fakebin:$toolbin" AOS_STATE_OVERRIDE="$d/state" AOS_SUBAGENT_STATE_NM_TIMEOUT=1 "$CREW_STATE" feat-timeout)
...
test_scout_skips_run_lookup() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/scout-j.meta" "window=fm:aos-scout-j" "worktree=$d/wt" "kind=scout"
  # Even if a run existed on this branch, a scout must not read it.
  AOS_FAKE_AXI_STATUS="$(run_running fm/scout-j)"
  AOS_FAKE_BUSY=1
...
test_torn_down_worktree() {
  ...
  make_fakebin "$d" >/dev/null
  aos_write_meta "$d/state/gone-k.meta" "window=fm:aos-gone-k" "worktree=$d/no-such-worktree" "kind=ship"
  local out rc
...
echo "all aos_subagent_state tests passed"
