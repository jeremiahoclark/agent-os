#!/usr/bin/env bash
# Tests for bin/aos-teardown.sh's landed-work safety check.
#
...
#   (m) aos-pr-check rerun after HEAD moved                      -> no stale pr_head
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TEARDOWN="$ROOT/bin/aos-teardown.sh"
PR_CHECK="$ROOT/bin/aos-pr-check.sh"
TMP_ROOT=$(fm_test_tmproot aos-teardown-tests)

# Build a fresh sandbox for one test case. Sets up:
#   $CASE/state/        - maestro state dir (with a fresh watcher beacon)
...
#   $CASE/project/      - clone of origin; acts as the maestro project dir
...
SH
  ...
  # Add a worktree on a fresh task branch; that branch is where the subagent commits.
  git -C "$case_dir/project" worktree add -q -b fm/task-x1 "$case_dir/wt" main

  # Fresh watcher beacon so aos-guard stays quiet.
...
write_meta() {
  ...
  aos_write_meta "$case_dir/state/task-x1.meta" \
    "window=aos-task-x1" \
...
add_fork_with_pushed_branch() {
  ...
  # so refs/remotes/fork/aos-task-x1 is visible from the worktree (shared object db).
...
run_teardown() {
  ...
  AOS_ROOT_OVERRIDE="$ROOT" \
  AOS_STATE_OVERRIDE="$case_dir/state" \
...
test_pr_check_does_not_refresh_stale_pr_head() {
  ...
  pass "aos-pr-check does not refresh PR head after HEAD moves"
}
