#!/usr/bin/env bash
# tests/aos_secondary_subagent_safety.test.sh - secondary_subagent home safety invariants:
...
# operator flow lives in aos_secondary_subagent_lifecycle_e2e.test.sh; this file keeps the
# destructive-invariant coverage that an e2e run cannot deterministically reach.
set -u

# shellcheck source=tests/secondary_subagent_helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/secondary_subagent_helpers.sh"

TMP_ROOT=$(fm_test_tmproot aos_secondary_subagent_safety)
...
test_fm_home_parameterization() {
  ...
  out=$(AOS_HOME="$home_one" "$ROOT/bin/aos-project-mode.sh" app)
  [ "$out" = "local-only on" ] || fail "aos-project-mode did not read projects.md from AOS_HOME"
  out=$(AOS_HOME="$home_two" "$ROOT/bin/aos-project-mode.sh" app 2>/dev/null)
  [ "$out" = "no-mistakes off" ] || fail "aos-project-mode did not isolate missing registry by home"

  AOS_HOME="$home_one" "$ROOT/bin/aos-brief.sh" task-a app >/dev/null || fail "brief scaffold failed under AOS_HOME"
  brief="$home_one/data/task-a/brief.md"
  [ -f "$brief" ] || fail "brief was not written under AOS_HOME/data"
  grep -F ">> '$home_one/state/task-a.status'" "$brief" >/dev/null || fail "brief did not shell-quote AOS_HOME state path"

  AOS_HOME="$home_one" "$ROOT/bin/aos-brief.sh" task-b app --scout >/dev/null || fail "scout brief scaffold failed under AOS_HOME"
  brief="$home_one/data/task-b/brief.md"
  grep -F ">> '$home_one/state/task-b.status'" "$brief" >/dev/null || fail "scout brief did not shell-quote AOS_HOME state path"

  AOS_HOME="$home_one" AOS_SECONDARY_SUBAGENT_CHARTER='ops domain' "$ROOT/bin/aos-brief.sh" task-c --secondary_subagent app >/dev/null \
    || fail "secondary_subagent brief scaffold failed under AOS_HOME"
  brief="$home_one/data/task-c/brief.md"
  grep -F ">> '$home_one/state/task-c.status'" "$brief" >/dev/null || fail "secondary_subagent brief did not shell-quote AOS_HOME state path"

  printf 'project=x\n' > "$home_one/state/task-a.meta"
  AOS_HOME="$home_one" AOS_GUARD_GRACE=999999 "$ROOT/bin/aos-pr-check.sh" task-a https://github.com/example/repo/pull/1 >/dev/null 2>/dev/null \
    || fail "aos-pr-check failed under AOS_HOME"
  [ -f "$home_one/state/task-a.check.sh" ] || fail "pr check was not written under AOS_HOME/state"
  [ ! -e "$home_two/state/task-a.check.sh" ] || fail "pr check leaked into another home"
  pass "AOS_HOME parameterizes data and state paths"
}
...
test_lock_status_is_per_home() {
  ...
  printf '999999\n' > "$home_one/state/.lock"
  out=$(AOS_HOME="$home_one" "$ROOT/bin/aos-lock.sh" status)
  printf '%s\n' "$out" | grep -F 'lock: stale' >/dev/null || fail "home one lock status did not read its own lock"
  out=$(AOS_HOME="$home_two" "$ROOT/bin/aos-lock.sh" status)
  [ "$out" = "lock: free" ] || fail "home two lock status was affected by home one"
  pass "aos-lock status is scoped per home"
}

test_seed_allows_overlapping_clones_and_drops_owner() {
  # A project may appear in several secondary_subagents' (non-exclusive) clone lists; the
  ...
  # init, modes preserved - is asserted by aos_secondary_subagent_lifecycle_e2e.
...
EOF
  ...
  AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='feature design for alpha beta' \
    AOS_SECONDARY_SUBAGENT_SCOPE='feature design for alpha beta' \
    "$ROOT/bin/aos-home-seed.sh" design "$design" alpha beta >/dev/null \
    || fail "initial seed failed"
  assert_grep '- design - feature design for alpha beta' "$home/data/secondary_subagents.md" "design registry line missing"
  assert_grep 'projects: alpha, beta' "$home/data/secondary_subagents.md" "design project clone list missing"
  assert_no_grep 'owns:' "$home/data/secondary_subagents.md" "registry used the legacy owns field"

  # beta is shared with a second secondary_subagent of a different scope (overlap allowed).
  AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='issue triage for beta' \
    AOS_SECONDARY_SUBAGENT_SCOPE='issue triage for beta' \
    "$ROOT/bin/aos-home-seed.sh" other "$other" beta >/dev/null 2>&1 \
    || fail "seed refused overlapping project clones across different scopes"
  assert_grep '- other - issue triage for beta' "$home/data/secondary_subagents.md" "overlapping registry line missing"
  AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" validate >/dev/null || fail "registry validation rejected overlapping clones"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" owner alpha >/dev/null 2>&1; then
...
test_home_seed_validate_rejects_duplicate_homes() {
  ...
  cat > "$home/data/secondary_subagents.md" <<EOF
...
EOF
  ...
  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" validate >/dev/null 2>"$err"; then
    fail "registry validation accepted two secondary_subagents with the same home"
  fi
  grep -F 'duplicate secondary_subagent home assignment' "$err" >/dev/null \
...
EOF
  ...
  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" validate >/dev/null 2>"$err"; then
    fail "registry validation accepted two homes for the same secondary_subagent id"
  fi
  grep -F 'duplicate secondary_subagent id assignment' "$err" >/dev/null \
...
EOF
  ...
  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" validate >/dev/null 2>"$err"; then
    fail "registry validation accepted nested secondary_subagent homes"
  fi
  grep -F 'overlapping secondary_subagent home assignment' "$err" >/dev/null \
...
test_home_seed_uses_treehouse_acquired_home() {
  ...
  out=$(PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TREEHOUSE_HOME="$acquired" AOS_FAKE_TMUX_LOG="$log" \
    AOS_FAKE_TREEHOUSE_LEASE_FILE="$lease" \
    AOS_SECONDARY_SUBAGENT_CHARTER='dash acquired scope' AOS_SECONDARY_SUBAGENT_SCOPE='dash acquired scope' \
    "$ROOT/bin/aos-home-seed.sh" dash - alpha) \
  ...
  grep -F 'treehouse get --lease --lease-holder dash' "$log" >/dev/null || fail "seed did not durably lease a home under the secondary_subagent id"
  [ -f "$lease" ] || fail "seed did not record a treehouse lease"
  [ "$(cat "$lease")" = dash ] || fail "seed did not set the lease holder to the secondary_subagent id"
  [ -f "$acquired/.aos_secondary_subagent_home" ] || fail "seed did not mark acquired home"
  [ "$(cat "$acquired/.aos_secondary_subagent_home")" = dash ] || fail "seed wrote wrong acquired-home marker"
  [ -d "$acquired/projects/alpha/.git" ] || fail "seed did not clone project into acquired home"
  grep -F "home: $acquired_abs" "$home/data/secondary_subagents.md" >/dev/null || fail "registry did not record acquired home"
  pass "home seeding durably leases treehouse-acquired dash homes under the secondary_subagent id"
}
...
test_home_seed_returns_treehouse_acquired_home_on_assignment_failure() {
  ...
  git clone --quiet "$ROOT" "$acquired"
  acquired_abs=$(cd "$acquired" && pwd -P)
  printf 'other\n' > "$acquired/.aos_secondary_subagent_home"
  fakebin=$(make_fake_tmux "$TMP_ROOT/dash-fail-fake")
  log="$TMP_ROOT/dash-fail-fake/tmux.log"

  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TREEHOUSE_HOME="$acquired" AOS_FAKE_TMUX_LOG="$log" \
    AOS_SECONDARY_SUBAGENT_CHARTER='dash acquired scope' AOS_SECONDARY_SUBAGENT_SCOPE='dash acquired scope' \
    "$ROOT/bin/aos-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
    fail "seed reused an acquired home marked for another secondary_subagent"
  fi
  ...
  if [ -f "$home/data/secondary_subagents.md" ] && grep -F -- '- dash ' "$home/data/secondary_subagents.md" >/dev/null; then
...
test_home_seed_warns_when_acquired_home_return_fails() {
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TREEHOUSE_HOME="$acquired" AOS_FAKE_TMUX_LOG="$log" \
    AOS_FAKE_TREEHOUSE_LEASE_FILE="$lease" AOS_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    AOS_SECONDARY_SUBAGENT_CHARTER='dash acquired scope' AOS_SECONDARY_SUBAGENT_SCOPE='dash acquired scope' \
    "$ROOT/bin/aos-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
...
test_home_seed_does_not_return_unsafe_acquired_home() {
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TREEHOUSE_HOME="$home" AOS_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/aos-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
    fail "seed accepted an acquired home matching the active maestro home"
  fi
  grep -F 'secondary_subagent home cannot be the active maestro home' "$err" >/dev/null \
  ...
  : > "$log"
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TREEHOUSE_HOME="$descendant" AOS_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/aos-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
    fail "seed accepted an acquired home inside the active maestro home"
  fi
  grep -F 'secondary_subagent home cannot be inside the active maestro home' "$err" >/dev/null \
...
EOF
  ...
  if AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='rollback scope' AOS_SECONDARY_SUBAGENT_SCOPE='rollback scope' \
    "$ROOT/bin/aos-home-seed.sh" rollback "$subhome" alpha beta >/dev/null 2>"$err"; then
  ...
  [ ! -e "$subhome" ] || fail "failed seed left the newly created secondary_subagent home behind"
  [ ! -e "$subhome/.aos_secondary_subagent_home" ] || fail "failed seed left a subhome marker"
  ...
  if [ -f "$home/data/secondary_subagents.md" ] && grep -F -- '- rollback ' "$home/data/secondary_subagents.md" >/dev/null; then
...
test_home_seed_refuses_missing_filled_charter() {
  ...
  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a direct seed without a filled charter"
  fi
  grep -F 'no filled secondary_subagent charter brief' "$err" >/dev/null \
...
test_home_seed_refuses_placeholder_charter() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  AOS_HOME="$home" "$ROOT/bin/aos-brief.sh" design --secondary_subagent alpha >/dev/null \
...
test_home_seed_refuses_empty_charter_fields() {
  ...
  if AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='   ' "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
  ...
  AOS_SECONDARY_SUBAGENT_SCOPE='   ' scaffold_secondary_subagent_charter "$home" design 'filled charter' alpha \
...
test_home_seed_refuses_local_only_project() {
  ...
  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed allowed a local-only project into a secondary_subagent home"
  fi
  grep -F 'project alpha is local-only; secondary_subagent routes support only no-mistakes and direct-PR projects' "$err" >/dev/null \
...
test_home_seed_refuses_registry_delimiter_home() {
  ...
  if AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='delimiter charter' "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a home path with registry delimiters"
  fi
  grep -F 'secondary_subagent home path contains registry delimiters' "$err" >/dev/null \
    || fail "seed did not explain delimiter home refusal"
  [ ! -e "$subhome/.aos_secondary_subagent_home" ] || fail "delimiter home seed wrote a marker"
  if [ -f "$home/data/secondary_subagents.md" ] && grep -F -- '- design ' "$home/data/secondary_subagents.md" >/dev/null; then
...
test_home_seed_refuses_active_home_and_root() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for active-home seed test"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$home" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondary_subagent home to reuse active AOS_HOME"
  fi
  grep -F 'secondary_subagent home cannot be the active maestro home' "$err" >/dev/null \
    || fail "seed did not explain active AOS_HOME rejection"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$active_descendant" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondary_subagent home inside active AOS_HOME"
  fi
  grep -F 'secondary_subagent home cannot be inside the active maestro home' "$err" >/dev/null \
    || fail "seed did not explain active AOS_HOME descendant rejection"
  [ ! -e "$home/nested" ] || fail "seed created a directory inside active AOS_HOME before descendant rejection"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$active_ancestor" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondary_subagent home to contain active AOS_HOME"
  fi
  grep -F 'secondary_subagent home cannot be an ancestor of the active maestro home' "$err" >/dev/null \
    || fail "seed did not explain active AOS_HOME ancestor rejection"
  [ ! -f "$active_ancestor/.aos_secondary_subagent_home" ] || fail "seed marked an ancestor of active AOS_HOME"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$ROOT" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondary_subagent home to reuse AOS_ROOT"
  fi
  grep -F 'secondary_subagent home cannot be the maestro repo' "$err" >/dev/null \
    || fail "seed did not explain AOS_ROOT rejection"

  git clone --quiet "$ROOT" "$root_clone"
  if AOS_HOME="$home" AOS_ROOT_OVERRIDE="$root_clone" "$ROOT/bin/aos-home-seed.sh" design "$root_descendant" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondary_subagent home inside AOS_ROOT"
  fi
  grep -F 'secondary_subagent home cannot be inside the maestro repo' "$err" >/dev/null \
    || fail "seed did not explain AOS_ROOT descendant rejection"
  [ ! -e "$root_clone/tmp" ] || fail "seed created a directory inside AOS_ROOT before descendant rejection"

  git clone --quiet "$ROOT" "$root_ancestor"
  git clone --quiet "$ROOT" "$root_inside"
  if AOS_HOME="$home" AOS_ROOT_OVERRIDE="$root_inside" "$ROOT/bin/aos-home-seed.sh" design "$root_ancestor" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondary_subagent home to contain AOS_ROOT"
  fi
  grep -F 'secondary_subagent home cannot be an ancestor of the maestro repo' "$err" >/dev/null \
    || fail "seed did not explain AOS_ROOT ancestor rejection"
  [ ! -f "$root_ancestor/.aos_secondary_subagent_home" ] || fail "seed marked an ancestor of AOS_ROOT"
...
test_home_seed_refuses_home_marked_for_another_id() {
  ...
  printf 'other\n' > "$subhome/.aos_secondary_subagent_home"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for marked-home seed test"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed reused a home marked for another secondary_subagent"
  fi
  grep -F 'already marked for other' "$err" >/dev/null || fail "seed did not explain marked-home rejection"
  [ "$(cat "$subhome/.aos_secondary_subagent_home")" = "other" ] || fail "seed overwrote another secondary_subagent marker"
...
test_home_seed_refuses_home_registered_to_another_id() {
  ...
  subhome_abs=$(cd "$subhome" && pwd -P)
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  printf '%s\n' '- other - other domain (home: '"$subhome_abs"'; scope: other domain; projects: beta; added 2026-06-22)' > "$home/data/secondary_subagents.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for registered-home seed test"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed reused a home registered to another secondary_subagent"
  fi
  grep -F 'already registered to other' "$err" >/dev/null || fail "seed did not explain registered-home rejection"
  [ ! -e "$subhome/.aos_secondary_subagent_home" ] || fail "seed wrote a marker before rejecting a registered home"
...
test_home_seed_refuses_reassigning_existing_id_to_different_home() {
  ...
  AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='design domain' AOS_SECONDARY_SUBAGENT_SCOPE='design domain' \
    "$ROOT/bin/aos-home-seed.sh" design "$first" alpha >/dev/null \
    || fail "initial seed failed for reassigning-id test"
  first_abs=$(cd "$first" && pwd -P)

  if AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='design domain' AOS_SECONDARY_SUBAGENT_SCOPE='design domain' \
    "$ROOT/bin/aos-home-seed.sh" design "$second" alpha >/dev/null 2>"$err"; then
    fail "seed reassigned an existing secondary_subagent id to a different home"
  fi
  grep -F "secondary_subagent id design is already registered to home $first_abs" "$err" >/dev/null \
    || fail "seed did not explain same-id different-home rejection"
  [ ! -e "$second" ] || fail "failed id reassignment created the new subhome"
  [ "$(cat "$first/.aos_secondary_subagent_home")" = design ] || fail "failed id reassignment changed the original marker"
  grep -F "home: $first_abs" "$home/data/secondary_subagents.md" >/dev/null \
  ...
  grep -F "home: $second_abs" "$home/data/secondary_subagents.md" >/dev/null \
...
EOF
  ...
  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$nested" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a home inside a registered secondary_subagent home"
  fi
  grep -F 'overlaps registered secondary_subagent home' "$err" >/dev/null \
    || fail "seed did not explain registered ancestor overlap"
  [ ! -e "$nested" ] || fail "seed created a nested home inside a registered home"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$parent" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a home containing a registered secondary_subagent home"
  fi
  grep -F 'overlaps registered secondary_subagent home' "$err" >/dev/null \
    || fail "seed did not explain registered descendant overlap"
  [ ! -f "$parent/.aos_secondary_subagent_home" ] || fail "seed marked a home containing a registered home"
...
test_home_seed_refuses_remote_backed_project_without_origin() {
  ...
  fm_git_init_commit "$home/projects/alpha"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for no-origin seed test"
...
test_home_seed_refuses_existing_remote_backed_project_with_wrong_origin() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for wrong-origin seed test"
...
test_home_seed_resolves_relative_source_origins() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for relative origin seed test"

  out=$(AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha)
  subhome_abs=$(cd "$subhome" && pwd -P)
  ...
  AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null \
...
test_home_seed_skips_initialized_existing_no_mistakes_projects() {
  ...
  if PATH="$fakebin:$PATH" AOS_FAKE_NO_MISTAKES_LOG="$log" AOS_FAKE_NO_MISTAKES_FAIL_PROJECT=beta \
    AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='existing init rollback scope' AOS_SECONDARY_SUBAGENT_SCOPE='existing init rollback scope' \
    "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha beta >/dev/null 2>"$err"; then
...
test_home_seed_refuses_uninitialized_existing_no_mistakes_project() {
  ...
  if PATH="$fakebin:$PATH" AOS_FAKE_NO_MISTAKES_LOG="$log" \
    AOS_HOME="$home" AOS_SECONDARY_SUBAGENT_CHARTER='existing uninitialized scope' \
    "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
...
test_home_seed_refuses_project_destinations_outside_subhome() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink destination seed test"

  if AOS_HOME="$home" "$ROOT/bin/aos-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed followed a subhome projects symlink outside the subhome"
  fi
  grep -F 'secondary_subagent projects directory must resolve inside the secondary_subagent home' "$err" >/dev/null \
  ...
  [ ! -f "$subhome/.aos_secondary_subagent_home" ] || fail "seed marked subhome after unsafe project destination rejection"
...
test_home_seed_refuses_operational_dirs_outside_subhome() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink operational dir seed test"
  ...
  for opdir in data state config; do
    ...
    grep -F "secondary_subagent $opdir directory must resolve inside the secondary_subagent home" "$err" >/dev/null \
      || fail "seed did not explain unsafe $opdir directory rejection"
    [ ! -f "$subhome/.aos_secondary_subagent_home" ] || fail "seed marked subhome after unsafe $opdir directory rejection"
  done
...
test_home_seed_refuses_symlinked_leaf_files() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondary_subagent_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink leaf seed test"

  for leaf in data/projects.md data/charter.md .aos_secondary_subagent_home; do
    ...
    expected=outside
    if [ "$leaf" = ".aos_secondary_subagent_home" ]; then
      expected=design
    fi
    printf '%s\n' "$expected" > "$sink"
    ...
    grep -F 'secondary_subagent leaf file must not be a symlink:' "$err" >/dev/null \
    ...
    [ ! -f "$subhome/.aos_secondary_subagent_home" ] || [ "$leaf" = ".aos_secondary_subagent_home" ] || fail "seed marked subhome after symlinked leaf refusal"
  done
  pass "home seeding refuses symlinked leaf files"
}

test_secondary_subagent_spawn_requires_seeded_matching_home() {
  ...
  cat > "$fakeroot/bin/aos-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$fakeroot/bin/aos-guard.sh"
  mkdir -p "$ancestor_active_home/data" "$ancestor_active_home/state" "$active_ancestor/data" "$root_ancestor/data" "$root_inside/bin"
  cat > "$root_inside/bin/aos-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$root_inside/bin/aos-guard.sh"
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$subhome" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted an unseeded home"
  fi
  grep -F 'not a seeded secondary_subagent home' "$err" >/dev/null || fail "spawn did not explain missing seed marker"
  ...
  printf 'other\n' > "$wronghome/.aos_secondary_subagent_home"
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$wronghome" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted a home marked for another secondary_subagent"
  fi
  grep -F 'marked for secondary_subagent other, expected domain' "$err" >/dev/null || fail "spawn did not explain marker mismatch"

  printf 'domain\n' > "$marker_only/.aos_secondary_subagent_home"
  printf 'charter\n' > "$marker_only/data/charter.md"
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$marker_only" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted a marked home missing AGENTS.md"
  fi
  grep -F 'not a maestro home (missing AGENTS.md)' "$err" >/dev/null || fail "spawn did not explain missing AGENTS.md"

  printf '# Maestro\n' > "$marker_only/AGENTS.md"
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$marker_only" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted a marked home missing bin"
  fi
  grep -F 'not a maestro home (missing bin/)' "$err" >/dev/null || fail "spawn did not explain missing bin"

  printf 'domain\n' > "$home/.aos_secondary_subagent_home"
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$home" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted the active home"
  fi
  grep -F 'secondary_subagent home cannot be the active maestro home' "$err" >/dev/null || fail "spawn did not reject active home"

  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$ROOT" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted the maestro repo root"
  fi
  grep -F 'secondary_subagent home cannot be the maestro repo' "$err" >/dev/null || fail "spawn did not reject maestro repo root"

  printf 'domain\n' > "$active_descendant/.aos_secondary_subagent_home"
  printf 'charter\n' > "$active_descendant/data/charter.md"
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$active_descendant" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted a home inside the active maestro home"
  fi
  grep -F 'secondary_subagent home cannot be inside the active maestro home' "$err" >/dev/null || fail "spawn did not reject active home descendant"

  printf 'domain\n' > "$active_ancestor/.aos_secondary_subagent_home"
  printf 'charter\n' > "$active_ancestor/data/charter.md"
  if PATH="$fakebin:$PATH" AOS_HOME="$ancestor_active_home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$active_ancestor" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted a home containing the active maestro home"
  fi
  grep -F 'secondary_subagent home cannot be an ancestor of the active maestro home' "$err" >/dev/null || fail "spawn did not reject active home ancestor"

  printf 'domain\n' > "$root_descendant/.aos_secondary_subagent_home"
  printf 'charter\n' > "$root_descendant/data/charter.md"
  if PATH="$fakebin:$PATH" AOS_ROOT_OVERRIDE="$fakeroot" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$root_descendant" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted a home inside the maestro repo"
  fi
  grep -F 'secondary_subagent home cannot be inside the maestro repo' "$err" >/dev/null || fail "spawn did not reject repo root descendant"

  printf 'domain\n' > "$root_ancestor/.aos_secondary_subagent_home"
  printf 'charter\n' > "$root_ancestor/data/charter.md"
  if PATH="$fakebin:$PATH" AOS_ROOT_OVERRIDE="$root_inside" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/aos-spawn.sh" domain "$root_ancestor" codex --secondary_subagent >/dev/null 2>"$err"; then
    fail "secondary_subagent spawn accepted a home containing the maestro repo"
  fi
  grep -F 'secondary_subagent home cannot be an ancestor of the maestro repo' "$err" >/dev/null || fail "spawn did not reject repo ancestor"

  pass "secondary_subagent spawn validates homes before launch"
}

test_secondary_subagent_spawn_refuses_operational_dirs_outside_subhome() {
  ...
  for opdir in data state config projects; do
    ...
    : > "$log"
    if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-opdir-fake/pane.txt" \
      "$ROOT/bin/aos-spawn.sh" domain "$subhome" codex --secondary_subagent >/dev/null 2>"$err"; then
      fail "secondary_subagent spawn accepted a subhome with $opdir symlinked outside the subhome"
    fi
    grep -F "secondary_subagent $opdir directory must resolve inside the secondary_subagent home" "$err" >/dev/null \
  ...
  pass "secondary_subagent spawn refuses operational directories outside the subhome"
}

test_fm_send_refuses_bare_window_without_home_meta() {
  # The happy path (a bare aos-<id> resolves the window recorded in THIS home's
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_WINDOW="other-session:aos-missing" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/send-fake/pane.txt" \
    "$ROOT/bin/aos-send.sh" aos-missing 'wrong home' >/dev/null 2>"$err"; then
    fail "aos-send sent to a bare maestro window without home metadata"
  fi
  grep -F "no metadata for aos-missing in $home/state" "$err" >/dev/null \
    || fail "aos-send did not explain missing home metadata"
  grep -F 'send-keys -t other-session:aos-missing' "$log" >/dev/null \
    && fail "aos-send fell back to a foreign same-name window"
  pass "aos-send refuses a bare maestro window with no metadata in this home"
}

test_secondary_subagent_teardown_retires_empty_home() {
  ...
  make_maestro_git_root "$fmroot"
  git -C "$fmroot" worktree add --quiet --detach "$subhome" HEAD
  mkdir -p "$home/state" "$home/data" "$subhome/state"
  printf 'domain\n' > "$subhome/.aos_secondary_subagent_home"
  subhome_abs=$(cd "$subhome" && pwd -P)
  cat > "$home/state/domain.meta" <<EOF
window=maestro:aos-domain
worktree=$subhome
project=$subhome
harness=echo
kind=secondary_subagent
mode=secondary_subagent
yolo=off
home=$subhome
projects=alpha
EOF
  printf '%s\n' '- domain - design domain (home: '"$subhome"'; scope: design domain; projects: alpha; added 2026-06-22)' > "$home/data/secondary_subagents.md"
  fakebin=$(make_fake_tmux "$TMP_ROOT/teardown-fake")
  log="$TMP_ROOT/teardown-fake/tmux.log"
  lease="$TMP_ROOT/teardown-fake/lease"
  printf 'domain\n' > "$lease"
  PATH="$fakebin:$PATH" AOS_ROOT_OVERRIDE="$fmroot" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-fake/pane.txt" \
    AOS_FAKE_TREEHOUSE_LEASE_FILE="$lease" \
    "$ROOT/bin/aos-teardown.sh" domain >/dev/null 2>/dev/null \
    || fail "teardown failed for empty secondary_subagent home"
  grep -F "treehouse return --force $subhome_abs" "$log" >/dev/null || fail "teardown did not release the secondary_subagent home lease via treehouse return"
  [ ! -e "$lease" ] || fail "teardown left the secondary_subagent home lease held after retirement"
  [ ! -d "$subhome" ] || fail "teardown did not remove the retired secondary_subagent home"
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta"
  grep -F -- '- domain ' "$home/data/secondary_subagents.md" >/dev/null && fail "teardown did not remove secondary_subagent registry route"
  pass "secondary_subagent teardown retires empty homes and releases routing"
}

test_secondary_subagent_teardown_refuses_failed_leased_home_return() {
...
EOF
  ...
  set +e
  PATH="$fakebin:$PATH" AOS_ROOT_OVERRIDE="$fmroot" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-return-fail-fake/pane.txt" \
    AOS_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    "$ROOT/bin/aos-teardown.sh" domain >/dev/null 2>"$err"
  rc=$?
  set -e
  ...
  grep -F 'treehouse return failed for secondary_subagent home' "$err" >/dev/null || fail "teardown did not report failed leased home return"
  ...
  grep -F -- '- domain ' "$home/data/secondary_subagents.md" >/dev/null || fail "teardown removed registry route after leased home return failed"
  pass "secondary_subagent teardown refuses to hide failed leased-home return"
}

test_secondary_subagent_teardown_removes_plain_clone_home_without_treehouse_return() {
...
EOF
  ...
  PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/plain-clone-teardown-fake/pane.txt" \
    AOS_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    "$ROOT/bin/aos-teardown.sh" domain >/dev/null 2>/dev/null \
    || fail "teardown failed for plain-clone secondary_subagent home"
  ...
  [ ! -d "$subhome" ] || fail "teardown did not remove the plain-clone secondary_subagent home"
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta for plain-clone home"
  grep -F -- '- domain ' "$home/data/secondary_subagents.md" >/dev/null && fail "teardown did not remove plain-clone registry route"
  pass "secondary_subagent teardown raw-removes plain-clone homes"
}

test_secondary_subagent_force_teardown_discards_child_work() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/force-teardown-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain >/dev/null 2>&1; then
    fail "teardown allowed a secondary_subagent with in-flight child work"
  fi
  PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/force-teardown-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain --force >/dev/null 2>/dev/null \
    || fail "force teardown failed to discard child work"
  [ ! -d "$subhome" ] || fail "force teardown did not remove the retired secondary_subagent home"
  [ ! -d "$childwt" ] || fail "force teardown did not remove child worktree"
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta"
  grep -F -- '- domain ' "$home/data/secondary_subagents.md" >/dev/null && fail "force teardown did not remove secondary_subagent registry route"
  grep -F 'kill-window -t maestro:aos-child' "$log" >/dev/null || fail "force teardown did not kill child window"
  grep -F 'kill-window -t maestro:aos-domain' "$log" >/dev/null || fail "force teardown did not kill parent window"
  pass "secondary_subagent force teardown discards child work"
}

test_secondary_subagent_force_teardown_allows_operational_dir_symlinks_inside_home() {
...
EOF
    ...
    PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/symlink-inside-teardown-fake-$opdir/pane.txt" \
      "$ROOT/bin/aos-teardown.sh" domain --force >/dev/null 2>"$err" \
      || fail "force teardown refused $opdir symlinked inside the secondary_subagent home"
    ...
    grep -F 'kill-window -t maestro:aos-domain' "$log" >/dev/null || fail "force teardown did not kill parent window for inside $opdir symlink"
  done
  pass "force teardown allows operational directory symlinks inside the subhome"
}

test_secondary_subagent_force_teardown_refuses_operational_dir_symlink_outside_home() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/symlink-state-teardown-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown accepted a symlinked secondary_subagent state directory"
  fi
  ...
  grep -F 'resolves outside the secondary_subagent home' "$err" >/dev/null || fail "teardown did not identify unsafe state symlink"
...
test_secondary_subagent_teardown_path_boundary_matrix() {
  # The teardown path-boundary matrix: a secondary_subagent home is refused (and left
  # fully intact, with no window killed before validation) when it is unmarked,
  # an ancestor of the active maestro home, inside the active maestro home,
  # or inside the maestro repo. One row per hazard, one shared assertion block.
  local row base home subhome fmroot fakebin log err expect tid
  while IFS='|' read -r row expect; do
    [ -n "$row" ] || continue
    base="$TMP_ROOT/td-pb-$row"
    fmroot="$ROOT"   # real maestro repo unless a row overrides it
    tid=domain
    case "$row" in
      unmarked)
        home="$base/main"; subhome="$base/sub"
        mkdir -p "$home/state" "$home/data" "$subhome/state"
        # No .aos_secondary_subagent_home marker on purpose.
        ;;
      ancestor)
        # The home being torn down is an ANCESTOR of the active maestro home.
      ...
      repo-descendant)
        ...
        cat > "$fmroot/bin/aos-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
        chmod +x "$fmroot/bin/aos-guard.sh"
        printf 'repo-domain\n' > "$subhome/.aos_secondary_subagent_home"
        ;;
    esac
    aos_write_secondary_subagent_meta "$home/state/$tid.meta" "$subhome"
    printf -- '- %s - design domain (home: %s; scope: design domain; projects: alpha; added 2026-06-22)\n' \
      "$tid" "$subhome" > "$home/data/secondary_subagents.md"
    fakebin=$(make_fake_tmux "$base/fake")
    log="$base/fake/tmux.log"
    err="$base/teardown.err"
    if PATH="$fakebin:$PATH" AOS_ROOT_OVERRIDE="$fmroot" AOS_HOME="$home" \
      AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$base/fake/pane.txt" \
      "$ROOT/bin/aos-teardown.sh" "$tid" >/dev/null 2>"$err"; then
      fail "teardown ($row) accepted a hazardous secondary_subagent home"
    fi
    ...
    grep -F -- "- $tid " "$home/data/secondary_subagents.md" >/dev/null || fail "teardown ($row) removed the registry route after refusal"
    grep -F 'kill-window' "$log" >/dev/null && fail "teardown ($row) killed a window before validation"
  done <<'ROWS'
unmarked|not a seeded secondary_subagent home
ancestor|ancestor of the active maestro home
active-descendant|inside the active maestro home
repo-descendant|inside the maestro repo
ROWS
  pass "secondary_subagent teardown path-boundary matrix refuses unmarked/ancestor/active-descendant/repo-descendant homes"
}

test_secondary_subagent_teardown_refuses_registered_nested_home() {
  ...
  printf 'domain\n' > "$subhome/.aos_secondary_subagent_home"
  printf 'nested\n' > "$nested/.aos_secondary_subagent_home"
  cat > "$home/state/domain.meta" <<EOF
window=maestro:aos-domain
worktree=$subhome
project=$subhome
...
EOF
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/nested-teardown-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain >/dev/null 2>"$err"; then
    fail "teardown removed a home containing another registered secondary_subagent home"
  fi
  ...
  grep -F 'contains registered secondary_subagent home' "$err" >/dev/null || fail "teardown did not explain registered nested-home refusal"
  pass "secondary_subagent teardown refuses homes containing registered nested homes"
}

test_secondary_subagent_teardown_refuses_child_registry_nested_home() {
  local home subhome nested fakebin err log
...
EOF
  printf '%s\n' '- domain - design domain (home: '"$subhome"'; scope: design domain; projects: alpha; added 2026-06-22)' > "$home/data/secondary_subagents.md"
  printf '%s\n' '- nested - nested domain (home: '"$nested"'; scope: nested domain; projects: beta; added 2026-06-22)' > "$subhome/data/secondary_subagents.md"
  fakebin=$(make_fake_tmux "$TMP_ROOT/child-registry-teardown-fake")
  log="$TMP_ROOT/child-registry-teardown-fake/tmux.log"
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-registry-teardown-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain >/dev/null 2>"$err"; then
    fail "teardown removed a home containing a child-registry secondary_subagent home"
  fi
  ...
  grep -F 'contains registered secondary_subagent home' "$err" >/dev/null || fail "teardown did not explain child-registry nested-home refusal"
  pass "secondary_subagent teardown refuses nested homes from the child registry"
}

test_secondary_subagent_force_teardown_prevalidates_before_child_cleanup() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/prevalidate-teardown-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain --force >/dev/null 2>"$err"; then
  ...
  grep -F 'not a seeded secondary_subagent home' "$err" >/dev/null || fail "force teardown did not explain missing seed marker"
  pass "force teardown validates subhome before child cleanup"
}

test_secondary_subagent_force_teardown_refuses_child_active_home_descendant() {
  local home subhome childproj childwt fakebin err log
...
EOF
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-active-descendant-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed a child worktree inside active AOS_HOME"
  fi
  ...
  grep -F 'inside the active maestro home' "$err" >/dev/null || fail "force teardown did not explain active home descendant rejection"
  pass "force teardown refuses child worktrees inside the active home"
}

test_secondary_subagent_force_teardown_refuses_child_repo_descendant() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" AOS_ROOT_OVERRIDE="$fakeroot" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-repo-descendant-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed a child worktree inside AOS_ROOT"
  fi
  ...
  grep -F 'inside the maestro repo' "$err" >/dev/null || fail "force teardown did not explain repo descendant rejection"
  pass "force teardown refuses child worktrees inside the maestro repo"
}

test_secondary_subagent_force_teardown_refuses_unregistered_child_worktree() {
  local home subhome childproj childwt fakebin err log
...
EOF
  ...
  if PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_LOG="$log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/unregistered-child-fake/pane.txt" \
    "$ROOT/bin/aos-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
test_secondary_subagent_idle_pane_is_not_stale() {
...
EOF
  ...
  PATH="$fakebin:$PATH" AOS_HOME="$home" AOS_FAKE_TMUX_WINDOW="$window" AOS_FAKE_TMUX_LOG="$TMP_ROOT/watch-fake/tmux.log" AOS_FAKE_TMUX_CAPTURE="$TMP_ROOT/watch-fake/pane.txt" \
    AOS_POLL=1 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$ROOT/bin/aos-watch.sh" > "$out" &
  pid=$!
  if ! wait_live "$pid" 25; then
    wait "$pid" || true
    grep -F "stale: $window" "$out" >/dev/null && fail "idle secondary_subagent pane triggered stale wake"
    fail "watcher exited unexpectedly while supervising idle secondary_subagent"
  fi
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  grep -F "stale: $window" "$out" >/dev/null && fail "idle secondary_subagent pane triggered stale wake"
  pass "idle kind=secondary_subagent pane is healthy and not stale"
}

test_secondary_subagent_charter_brief_is_idle_by_default() {
  local home brief
  home="$TMP_ROOT/idle-charter-home"
  mkdir -p "$home/data" "$home/state"
  scaffold_secondary_subagent_charter "$home" idle-sm 'feature work for alpha' alpha
  brief="$home/data/idle-sm/brief.md"
  [ -f "$brief" ] || fail "secondary_subagent charter brief was not scaffolded"
  # Idle contract: waits for routed work, never self-initiates.
  grep -F 'go idle and wait silently for the main maestro' "$brief" >/dev/null \
    || fail "charter brief does not tell the secondary_subagent to go idle and wait for routed work"
  grep -F 'Act only on tasks the main maestro routes to you' "$brief" >/dev/null \
  ...
  grep -F 'run normal maestro bootstrap and recovery' "$brief" >/dev/null \
  ...
  pass "secondary_subagent charter brief is idle by default and does not self-initiate work"
}
...
test_backlog_handoff_aborts_safely() {
  ...
  mkdir -p "$home/data" "$home/state"
  seed_secondary_subagent_home_marker "$subhome" design
  subhome_abs=$(cd "$subhome" && pwd -P)
  printf -- '- design - feature work (home: %s; scope: feature work; projects: alpha; added 2026-06-22)\n' "$subhome_abs" > "$home/data/secondary_subagents.md"
...
EOF
  ...
  if AOS_HOME="$home" "$ROOT/bin/aos-backlog-handoff.sh" design bug-z no-such-key >/dev/null 2>&1; then
  ...
  before=$(cat "$home/data/backlog.md")
  if AOS_HOME="$home" "$ROOT/bin/aos-backlog-handoff.sh" design live-task >/dev/null 2>&1; then
  ...
  [ ! -e "$subhome/data/backlog.md" ] || ! grep -F 'live-task' "$subhome/data/backlog.md" >/dev/null     || fail "in-flight refusal copied the live task into the secondary_subagent backlog"

  # An unregistered secondary_subagent id is refused.
  if AOS_HOME="$home" "$ROOT/bin/aos-backlog-handoff.sh" ghost bug-z >/dev/null 2>&1; then
    fail "handoff accepted an unregistered secondary_subagent id"
  fi
  pass "aos-backlog-handoff aborts atomically on unmatched, in-flight, and unregistered targets"
}

test_backlog_handoff_creates_absent_section_and_refuses_non_secondary_subagent_home() {
  ...
  # A Done item handed into a secondary_subagent backlog lacking a Done section gets one.
  seed_secondary_subagent_home_marker "$subhome" archive
  subhome_abs=$(cd "$subhome" && pwd -P)
...
EOF
  AOS_HOME="$home" "$ROOT/bin/aos-backlog-handoff.sh" archive shipped-task >/dev/null \
    || fail "handoff of a Done item failed"
  grep -F '## Done' "$subhome/data/backlog.md" >/dev/null \
    || fail "handoff did not create the missing Done section in the secondary_subagent backlog"
  ...
  grep -F 'keep-me' "$subhome/data/backlog.md" >/dev/null || fail "handoff clobbered the existing secondary_subagent backlog content"

  # A registered home that is not a seeded secondary_subagent home (e.g. a project clone)
  # is refused, and nothing is written into it.
  fm_git_init_commit "$projhome"
  projhome_abs=$(cd "$projhome" && pwd -P)
  printf -- '- proj-sm - bogus (home: %s; scope: bogus; projects: alpha; added 2026-06-22)\n' "$projhome_abs" >> "$home/data/secondary_subagents.md"
  if AOS_HOME="$home" "$ROOT/bin/aos-backlog-handoff.sh" proj-sm shipped-task >/dev/null 2>&1; then
    fail "handoff wrote into a destination that is not a seeded secondary_subagent home"
  fi
  [ ! -e "$projhome/data/backlog.md" ] || fail "handoff created a backlog inside a non_secondary_subagent home"

  mkdir -p "$markerhome/data"
  markerhome_abs=$(cd "$markerhome" && pwd -P)
  printf 'marker-sm\n' > "$markerhome/.aos_secondary_subagent_home"
  printf -- '- marker-sm - bogus (home: %s; scope: bogus; projects: alpha; added 2026-06-22)\n' "$markerhome_abs" >> "$home/data/secondary_subagents.md"
  cat > "$home/data/backlog.md" <<'EOF'
## Queued
- [ ] marker-task - should not move (repo: alpha)
EOF
  if AOS_HOME="$home" "$ROOT/bin/aos-backlog-handoff.sh" marker-sm marker-task >/dev/null 2>&1; then
    fail "handoff accepted a marker-only directory as a secondary_subagent home"
  fi
  ...
  seed_secondary_subagent_home_marker "$symlinkhome" symlink-sm
  symlinkhome_abs=$(cd "$symlinkhome" && pwd -P)
  mkdir -p "$outside"
  rm -rf "$symlinkhome/data"
  ln -s "$outside" "$symlinkhome/data"
  printf -- '- symlink-sm - bogus (home: %s; scope: bogus; projects: alpha; added 2026-06-22)\n' "$symlinkhome_abs" >> "$home/data/secondary_subagents.md"
  cat > "$home/data/backlog.md" <<'EOF'
## Queued
- [ ] symlink-task - should not move (repo: alpha)
EOF
  if AOS_HOME="$home" "$ROOT/bin/aos-backlog-handoff.sh" symlink-sm symlink-task >/dev/null 2>&1; then
    fail "handoff accepted a secondary_subagent home with data outside the home"
  fi
  [ ! -e "$outside/backlog.md" ] || fail "handoff wrote through a symlinked secondary_subagent data directory"
  grep -F 'symlink-task' "$home/data/backlog.md" >/dev/null || fail "symlink refusal lost the main backlog item"
  pass "aos-backlog-handoff creates absent sections and refuses unsafe homes"
}
