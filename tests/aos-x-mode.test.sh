#!/usr/bin/env bash
# Behavior tests for X mode: the relay poll client (aos-x-poll.sh), the answer
# poster (aos-x-reply.sh), and bootstrap's .env-presence activation.
#
...
BASE_PATH=${AOS_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
...
TMP_ROOT=$(fm_test_tmproot aos-x-mode-tests)
...
test_poll_no_token_is_hard_noop() {
  ...
  # No .env, no AOSX_PAIRING_TOKEN: must exit 0 with no output and touch nothing.
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_PAIRING_TOKEN='' \
    "$ROOT/bin/aos-x-poll.sh"); rc=$?
  ...
  pass "aos-x-poll is a hard no-op without a token (inert default)"
}
...
test_poll_empty_env_token_overrides_env_file() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'AOSX_PAIRING_TOKEN=tok-dotenv\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_PAIRING_TOKEN='' \
  ...
  pass "aos-x-poll treats an explicitly empty env token as configured"
}
...
test_poll_204_is_silent() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'AOSX_PAIRING_TOKEN=tok-204\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_POLL_CODE=204 \
    "$ROOT/bin/aos-x-poll.sh"); rc=$?
  ...
  pass "aos-x-poll stays silent on HTTP 204 (the common case)"
}
...
test_poll_empty_env_relay_overrides_env_file() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'AOSX_PAIRING_TOKEN=tok-relay\nAOSX_RELAY_URL=https://dotenv-relay.test/\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL='' \
    FAKE_CURL_LOG="$log" FAKE_POLL_CODE=204 \
    "$ROOT/bin/aos-x-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll empty-env-relay exit"
  [ -z "$out" ] || fail "poll 204 with empty env relay must be silent (got: $out)"
  assert_grep "url=https://mymaestro.io/connector/poll" "$log" \
    "empty env relay must override .env and fall back to the default relay"
  pass "aos-x-poll lets an explicitly empty relay env override .env"
}
...
test_poll_auth_error_reports_once() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'AOSX_PAIRING_TOKEN=tok-auth\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=401 \
    "$ROOT/bin/aos-x-poll.sh"); rc=$?
  ...
  pass "aos-x-poll surfaces auth/config errors once and clears on recovery"
}
...
test_poll_question_stashes_and_marks() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'AOSX_PAIRING_TOKEN=tok-q\n' > "$home/.env"
  ...
  pass "aos-x-poll stashes the question and prints the compact marker"
}
...
test_poll_preserves_conversation_context() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'AOSX_PAIRING_TOKEN=tok-c\n' > "$home/.env"
  ...
  pass "aos-x-poll preserves in_reply_to conversation context in the inbox"
}
...
SH
  ...
  pass "aos-x-poll reports inbox commit failures without emitting a mention wake"
}
...
test_poll_rejects_unsafe_request_id() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'AOSX_PAIRING_TOKEN=tok-e\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
  ...
  pass "aos-x-poll rejects an unsafe request_id (path-traversal guard)"
}
...
test_reply_success_posts_request_bound_only() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'AOSX_PAIRING_TOKEN=tok-r\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/aos-x-reply.sh" "req-7" "Got it, charting a couple of fixes."); rc=$?
  ...
  [ "$(printf '%s' "$data" | jq -r .text)" = "Got it, charting a couple of fixes." ] || fail "reply body text"
  keys=$(printf '%s' "$data" | jq -r 'keys|join(",")')
  [ "$keys" = "request_id,text" ] || fail "reply body must carry only request_id,text (got: $keys)"
  pass "aos-x-reply posts a request-bound answer and echoes only the request_id"
}
...
test_reply_non_2xx_fails() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/aos-x-reply.sh" "req-7" "hi" 2>"$err"); rc=$?
  [ "$rc" -ne 0 ] || fail "reply must exit non-zero on a non-2xx response"
  assert_grep "HTTP 500" "$err" "reply must report the failing status"
  pass "aos-x-reply exits non-zero on a non-2xx relay response"
}

test_reply_usage_error() {
  local home rc
  home="$TMP_ROOT/reply-usage"; mkdir -p "$home"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-reply.sh" "only-one" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "reply usage error exit"
  pass "aos-x-reply rejects missing arguments with a usage error"
}
...
test_reply_whitespace_text_rejected() {
  ...
  err="$home/err.txt"
  out=$(PATH="$BASE_PATH" AOS_HOME="$home" AOSX_DRY_RUN=1 \
    "$ROOT/bin/aos-x-reply.sh" "req-space" "   " 2>"$err"); rc=$?
  ...
  pass "aos-x-reply rejects whitespace-only reply text"
}
...
test_bootstrap_activates_on_env_token() {
  ...
  assert_grep "export AOS_CHECK_INTERVAL=30" "$home/config/x-mode.env" "cadence must be 30s"
  # Cadence inheritance: sourcing the config exports the 30s interval to a child,
  # exactly how aos-watch-arm.sh's forked watcher inherits it.
  local inherited
  # shellcheck source=/dev/null
  inherited=$( . "$home/config/x-mode.env" && bash -c 'echo "${AOS_CHECK_INTERVAL:-300}"' )
  [ "$inherited" = "30" ] \
    || fail "sourcing the cadence config must export AOS_CHECK_INTERVAL=30 to a child"
  ...
  AOS_HOME="$home" "$ROOT/bin/aos-bootstrap.sh" >/dev/null 2>&1
...
SH
  ...
  printf 'AOSX_PAIRING_TOKEN=tok-missing\n' > "$home/.env"
...
test_poll_empty_text_is_silent() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'AOSX_PAIRING_TOKEN=tok-t\n' > "$home/.env"
  ...
  pass "aos-x-poll requires a non-empty question before waking"
}
...
test_reply_text_file_and_stdin() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/aos-x-reply.sh" "req-1" --text-file "$home/reply.txt"); rc=$?
  ...
  # stdin form.
  log="$home/stdin.log"
  out=$(printf '%s' 'reply via stdin' | PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" \
    AOSX_RELAY_URL="https://relay.test" FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/aos-x-reply.sh" "req-2" -); rc=$?
  expect_code 0 "$rc" "reply stdin exit"
  data=$(grep '^data=' "$log" | tail -1 | sed 's/^data=//')
  [ "$(printf '%s' "$data" | jq -r .text)" = 'reply via stdin' ] \
    || fail "reply via stdin must send the piped text"
  pass "aos-x-reply accepts the reply via --text-file and stdin (safe, unexpanded)"
}
...
test_bootstrap_opt_out_cleanup() {
  ...
  printf 'AOSX_PAIRING_TOKEN=tok-out\n' > "$home/.env"
  AOS_HOME="$home" "$ROOT/bin/aos-bootstrap.sh" >/dev/null 2>&1
...
test_reply_dry_run_records_not_posts() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'AOSX_PAIRING_TOKEN=tok-d\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    AOSX_DRY_RUN=1 FAKE_CURL_LOG="$log" \
    "$ROOT/bin/aos-x-reply.sh" "req-1" "Got it, a couple of fixes underway." 2>"$home/err"); rc=$?
  ...
  [ "$(jq -r .text "$home/state/x-outbox/req-1.json")" = "Got it, a couple of fixes underway." ] \
  ...
  pass "aos-x-reply dry-run records the would-be reply and never posts"
}
...
test_reply_dry_run_needs_no_token() {
  ...
  out=$(PATH="$BASE_PATH" AOS_HOME="$home" AOSX_DRY_RUN=1 \
    "$ROOT/bin/aos-x-reply.sh" "req-2" "preview without creds" 2>/dev/null); rc=$?
  ...
  pass "aos-x-reply dry-run works without a token"
}
...
test_reply_dry_run_from_env_file() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  # AOSX_DRY_RUN read from .env (not just the environment).
  printf 'AOSX_PAIRING_TOKEN=tok-d\nAOSX_DRY_RUN=1\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" "$ROOT/bin/aos-x-reply.sh" "req-3" "from dotenv" 2>/dev/null); rc=$?
  ...
  pass "aos-x-reply honors AOSX_DRY_RUN from .env"
}
...
test_reply_empty_env_dry_run_overrides_env_file() {
  ...
  printf 'AOSX_PAIRING_TOKEN=tok-d\nAOSX_DRY_RUN=1\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    AOSX_DRY_RUN='' FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/aos-x-reply.sh" "req-5" "empty env disables dry run" 2>/dev/null); rc=$?
  ...
  pass "aos-x-reply lets an explicitly empty dry-run env override .env"
}
...
test_reply_dry_run_fails_when_outbox_unwritable() {
  ...
  out=$(PATH="$BASE_PATH" AOS_HOME="$home" AOSX_DRY_RUN=1 \
    "$ROOT/bin/aos-x-reply.sh" "req-4" "preview text" 2>"$err"); rc=$?
  ...
  pass "aos-x-reply dry-run fails when it cannot record the preview"
}

test_split_thread_lib() {
  # shellcheck source=bin/aos-x-lib.sh
  . "$ROOT/bin/aos-x-lib.sh"
  local out n last rejoin maxlen txt
  # A reply that fits one tweet stays a single, UNNUMBERED chunk.
  out=$(printf 'Got it, all solid.' | fmx_split_thread 280 25)
  [ "$(printf '%s' "$out" | jq 'length')" = "1" ] || fail "short reply must be one chunk"
  [ "$(printf '%s' "$out" | jq -r '.[0]')" = "Got it, all solid." ] || fail "short reply must be verbatim and unnumbered"
...
test_reply_single_no_texts() {
  ...
  out=$(AOS_HOME="$home" AOSX_DRY_RUN=1 "$ROOT/bin/aos-x-reply.sh" req-s "Short and sweet." 2>/dev/null)
  ...
  pass "aos-x-reply keeps a concise reply as a single unnumbered tweet"
}

test_reply_thread_dry_run() {
  local home out long
  home="$TMP_ROOT/reply-thread"; mkdir -p "$home"
  long="The owner has me on a sign-in redirect fix, a docs tidy, and keeping the build green while other jobs run in the background today."
  out=$(AOS_HOME="$home" AOSX_DRY_RUN=1 AOSX_X_REPLY_MAX_CHARS=50 \
    "$ROOT/bin/aos-x-reply.sh" req-t "$long" 2>/dev/null)
  ...
  pass "aos-x-reply auto-splits a long reply into a numbered thread (texts[])"
}
...
test_reply_max_chars_floor_clamps_to_minimum() {
  ...
  out=$(AOS_HOME="$home" AOSX_DRY_RUN=1 AOSX_X_REPLY_MAX_CHARS=49 \
    "$ROOT/bin/aos-x-reply.sh" req-floor "$long" 2>/dev/null)
  ...
  pass "aos-x-reply clamps a below-floor max to 50 characters"
}
...
test_reply_thread_live_posts_texts() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'AOSX_PAIRING_TOKEN=tok-th\n' > "$home/.env"
  # 50 is the configured minimum per-tweet budget; the text is well over it so it
  # must split into a multi-tweet thread.
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    AOSX_X_REPLY_MAX_CHARS=50 FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/aos-x-reply.sh" req-l "alpha bravo charlie delta echo foxtrot golf hotel india juliet kilo lima mike november oscar papa quebec romeo")
  ...
  pass "aos-x-reply posts a thread payload (texts[]) to the relay"
}
...
test_reply_followup_live_posts_to_followup_endpoint() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'AOSX_PAIRING_TOKEN=tok-fu\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/aos-x-reply.sh" "req-7" --followup "Done, owner - the fix has shipped."); rc=$?
  ...
  pass "aos-x-reply --followup posts to /connector/followup with the same request-bound body"
}
...
test_reply_followup_flag_position_is_flexible() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/aos-x-reply.sh" "req-a" --text-file "$home/reply.txt"); rc=$?
  ...
  pass "aos-x-reply --followup is accepted in any position and leaves the answer path default"
}

test_reply_followup_dry_run_marks_endpoint() {
  local home out rc
  home="$TMP_ROOT/reply-followup-dry"; mkdir -p "$home"
  out=$(AOS_HOME="$home" AOSX_DRY_RUN=1 \
    "$ROOT/bin/aos-x-reply.sh" "req-d" --followup "Shipped - all green." 2>"$home/err"); rc=$?
  ...
  out=$(AOS_HOME="$home" AOSX_DRY_RUN=1 "$ROOT/bin/aos-x-reply.sh" "req-ans" "Aye." 2>/dev/null)
  ...
  pass "aos-x-reply --followup dry-run marks the endpoint without changing the answer path"
}
...
test_reply_followup_thread_dry_run() {
  ...
  long="The owner has me on a sign-in redirect fix, a docs tidy, and keeping the build green while other jobs run in the background today."
  out=$(AOS_HOME="$home" AOSX_DRY_RUN=1 AOSX_X_REPLY_MAX_CHARS=50 \
    "$ROOT/bin/aos-x-reply.sh" req-ft --followup "$long" 2>/dev/null)
  ...
  pass "aos-x-reply --followup auto-splits a long follow-up into a marked thread"
}

# --- aos-x-dismiss: drop a mention at the relay without replying ---------------
...
test_dismiss_success_posts_request_only() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/aos-x-dismiss.sh" "req-9"); rc=$?
  ...
  pass "aos-x-dismiss posts a request-bound dismiss and echoes only the request_id"
}
...
test_dismiss_dry_run_records_not_posts() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    AOSX_DRY_RUN=1 FAKE_CURL_LOG="$log" \
    "$ROOT/bin/aos-x-dismiss.sh" "req-1" 2>"$home/err"); rc=$?
  ...
  pass "aos-x-dismiss dry-run records the would-be body and never posts"
}
...
test_dismiss_dry_run_needs_no_token() {
  ...
  # No token at all: dry-run still previews (it neither authenticates nor posts).
  out=$(PATH="$BASE_PATH" AOS_HOME="$home" AOSX_DRY_RUN=1 \
    "$ROOT/bin/aos-x-dismiss.sh" "req-2" 2>/dev/null); rc=$?
  ...
  pass "aos-x-dismiss dry-run works without a token"
}
...
test_dismiss_non_2xx_fails() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/aos-x-dismiss.sh" "req-9" 2>"$err"); rc=$?
  ...
  pass "aos-x-dismiss exits non-zero on a non-2xx relay response"
}
...
SH
  ...
  pass "aos-x-dismiss exits non-zero on a transport failure"
}
...
test_dismiss_unsafe_request_id_rejected() {
  ...
  out=$(PATH="$BASE_PATH" AOS_HOME="$home" AOSX_DRY_RUN=1 \
    "$ROOT/bin/aos-x-dismiss.sh" "../evil" 2>"$err"); rc=$?
  ...
  pass "aos-x-dismiss rejects an unsafe request_id (path-traversal guard)"
}

test_dismiss_usage_error() {
  local home rc
  home="$TMP_ROOT/dismiss-usage"; mkdir -p "$home"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-dismiss.sh" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss missing-arg usage exit"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-dismiss.sh" req-1 extra >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss extra-arg usage exit"
  pass "aos-x-dismiss rejects missing or extra arguments with a usage error"
}

# --- aos-x-link: task <-> X-request association in meta -----------------------
...
test_link_records_request_and_timestamp() {
  ...
  printf 'window=w\nworktree=/wt\nkind=ship\nmode=no-mistakes\nyolo=off\n' > "$meta"
  out=$(AOS_HOME="$home" AOSX_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/aos-x-link.sh" fix-login-k3 req-42); rc=$?
  ...
  AOS_HOME="$home" AOSX_NOW_OVERRIDE=1700009999 "$ROOT/bin/aos-x-link.sh" fix-login-k3 req-99 >/dev/null
  ...
  pass "aos-x-link records and refreshes the X-request link without disturbing meta"
}
...
test_meta_rewrites_do_not_depend_on_tmpdir() {
  ...
  printf 'window=w\nkind=ship\n' > "$meta"
  out=$(TMPDIR="$badtmp" AOS_HOME="$home" AOSX_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/aos-x-link.sh" fix-meta-k4 req-local); rc=$?
  ...
  out=$(TMPDIR="$badtmp" AOS_HOME="$home" AOSX_NOW_OVERRIDE=1700000001 AOSX_FOLLOWUP_MAX_AGE_SECS=0 \
    "$ROOT/bin/aos-x-followup.sh" --check fix-meta-k4 2>/dev/null); rc=$?
...
test_link_rejects_unsafe_and_missing() {
  ...
  printf 'kind=ship\n' > "$home/state/ok.meta"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-link.sh" "../evil" req-1 >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link unsafe task id exit"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-link.sh" ok "../../etc/x" >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-link.sh" no-such req-1 >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-link.sh" ok >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link missing arg exit"
  pass "aos-x-link rejects unsafe ids, missing meta, and missing arguments"
}

# --- aos-x-followup: detect, post one follow-up, clear the link ---------------
...
mk_linked_task() { # <home> <id> <request_id> <link-epoch>
  ...
  printf 'window=w\nworktree=/wt\nkind=ship\nmode=no-mistakes\nyolo=off\n' > "$meta"
  AOS_HOME="$home" AOSX_NOW_OVERRIDE="$ts" "$ROOT/bin/aos-x-link.sh" "$id" "$rid" >/dev/null
}
...
test_followup_check_states() {
  ...
  out=$(AOS_HOME="$home" AOSX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/aos-x-followup.sh" --check task-a); rc=$?
  ...
  out=$(AOS_HOME="$home" "$ROOT/bin/aos-x-followup.sh" --check plain 2>/dev/null); rc=$?
  ...
  out=$(AOS_HOME="$home" "$ROOT/bin/aos-x-followup.sh" --check nope 2>/dev/null); rc=$?
  expect_code 1 "$rc" "check missing-meta exit"
  pass "aos-x-followup --check reports postable / not-linked correctly"
}
...
test_followup_check_expired_prunes_link() {
  ...
  out=$(AOS_HOME="$home" AOSX_NOW_OVERRIDE=$((1700000000 + 25*3600)) \
    "$ROOT/bin/aos-x-followup.sh" --check task-e 2>/dev/null); rc=$?
  ...
  pass "aos-x-followup --check prunes a link past the 24h window"
}
...
test_followup_post_within_window_posts_and_clears() {
  ...
  printf 'Done, owner - shipped and green.' > "$home/reply.txt"
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    AOSX_NOW_OVERRIDE=1700003600 FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/aos-x-followup.sh" task-p --text-file "$home/reply.txt"); rc=$?
  ...
  data=$(grep '^data=' "$log" | tail -1 | sed 's/^data=//')
  [ "$(printf '%s' "$data" | jq -r .text)" = "Done, owner - shipped and green." ] \
  ...
  pass "aos-x-followup posts the follow-up and clears the link on success"
}
...
test_followup_post_failure_keeps_link() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    AOSX_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=500 \
    "$ROOT/bin/aos-x-followup.sh" task-f - <<<"retry me" 2>/dev/null); rc=$?
  ...
  pass "aos-x-followup keeps the link when the post fails"
}
...
test_followup_post_expired_skips_and_clears() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AOS_HOME="$home" AOSX_RELAY_URL="https://relay.test" \
    AOSX_NOW_OVERRIDE=$((1700000000 + 90000)) FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/aos-x-followup.sh" task-x - <<<"too late" 2>/dev/null); rc=$?
  ...
  pass "aos-x-followup skips silently and clears the link past the 24h window"
}
...
test_followup_post_not_linked_is_noop() {
  ...
  printf 'kind=ship\n' > "$home/state/plain.meta"
  out=$(AOS_HOME="$home" "$ROOT/bin/aos-x-followup.sh" plain - <<<"nothing to do" 2>/dev/null); rc=$?
  ...
  pass "aos-x-followup is a no-op for a task with no X link"
}
...
test_followup_post_dry_run_records_and_clears() {
  ...
  out=$(AOS_HOME="$home" AOSX_DRY_RUN=1 AOSX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/aos-x-followup.sh" task-d - <<<"Shipped in dry run." 2>/dev/null); rc=$?
  ...
  pass "aos-x-followup dry-run records the follow-up and clears the link"
}

test_followup_usage_errors() {
  local home rc
  home="$TMP_ROOT/fu-usage"; mkdir -p "$home/state"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-followup.sh" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup no-args exit"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-followup.sh" --check >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup --check no-id exit"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-followup.sh" some-task >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup post no-text-source exit"
  PATH="$BASE_PATH" AOS_HOME="$home" "$ROOT/bin/aos-x-followup.sh" "../evil" --text-file /dev/null >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup unsafe-id exit"
  pass "aos-x-followup rejects malformed invocations"
}
