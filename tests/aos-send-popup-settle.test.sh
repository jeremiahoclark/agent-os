#!/usr/bin/env bash
# aos-send pre-submit popup-settle selection (the codex `$<skill>` fix).
#
...
# popup settles lets it swallow the Enter, so the line never submits. aos-send
...
# aos-send's own post-submit AOS_SEND_SETTLE pause. So tail-vs-head matters: this
# suite asserts on the HEAD sleep, distinct from aos-send-settle.test.sh which pins
...
TMP_ROOT=$(fm_test_tmproot aos-send-popup-settle)

# Same stub shape as aos-send-settle.test.sh: a fake tmux that drives the submit
# path to a clean "empty" verdict on the first Enter, and a fake sleep that records
# every requested duration (one per line) into AOS_SLEEP_LOG instead of sleeping.
...
# (the popup-settle) equals <expected>. AOS_SEND_SETTLE=0 strips the trailing
# post-submit pause so the log holds only the popup-settle plus the 0.4 Enter wait,
# keeping the head assertion crisp. AOS_ROOT_OVERRIDE points at a non-repo dir so
# aos-guard's tangle check stays silent; its watcher-liveness note goes to stderr
# (discarded).
first_settle() {  # <expected> <label> <harness|--explicit> <message>
  ...
  if [ "$harness" = --explicit ]; then
    ...
    target="aos-popupcase"
    aos_write_meta "$home/state/popupcase.meta" "window=sess:win" "harness=$harness"
  fi
  : > "$log"
  env AOS_SEND_SETTLE=0 PATH="$fb:$PATH" \
  ...
  pass "aos-send popup-settle: $label -> ${expected}s"
}
