...
#
# Maestro is a treehouse-pooled git repo of itself: linked worktrees and
# secondary_subagent homes all sit at a detached HEAD on the default branch, while the
# PRIMARY checkout (AOS_ROOT) is a normal checkout on a real branch. The "tangle"
# is a subagent branching/committing in the primary instead of its own worktree,
...
#            aos-spawn refuses to launch unless the resolved worktree is isolated.
#   GUARD 2 (detection)  - aos-guard and aos-bootstrap alarm when the primary is on
#            a feature branch, and stay silent on the default branch or detached.
# These cases pin: the shared lib's branch classification, the aos-guard banner,
# the aos-bootstrap problem line, the brief assertion ordering, and the aos-spawn
...
. "$ROOT/bin/aos-tangle-lib.sh"

TMP_ROOT=$(fm_test_tmproot aos-tangle-guard)
...
detached HEAD on default is healthy (worktrees, secondary_subagent homes)|detached||
ROWS
...
# --- GUARD 2a: aos-guard banner ----------------------------------------------

run_guard() {
  # Scope the guard to a temp repo as the primary checkout; state lives under it.
  AOS_ROOT_OVERRIDE="$1" AOS_HOME="$1" "$ROOT/bin/aos-guard.sh" 2>&1
}
...
test_guard_banner() {
  ...
  pass "aos-guard: bordered tangle banner fires only for a feature branch in the primary"
}

# --- GUARD 2b: aos-bootstrap problem line ------------------------------------

run_bootstrap() {
  # No projects/ under the home keeps fleet sync inert; grep isolates the line.
  AOS_ROOT_OVERRIDE="$1" AOS_HOME="$1" "$ROOT/bin/aos-bootstrap.sh" 2>/dev/null
}
...
test_bootstrap_line() {
  ...
  pass "aos-bootstrap: TANGLE problem line fires only for a feature branch in the primary"
}

# --- GUARD 1a: brief isolation assertion ------------------------------------

# The generated ship brief must carry the isolation assertion AHEAD of the
# `git checkout -b` step, so the subagent verifies its worktree before branching.
test_brief_assertion_precedes_branch() {
  local home brief iso br
  home="$TMP_ROOT/brief-home"
  mkdir -p "$home/data"
  AOS_HOME="$home" "$ROOT/bin/aos-brief.sh" tangle-brief-cc3 alpha >/dev/null 2>&1
  ...
  pass "aos-brief: ship brief asserts worktree isolation before the branch step"
}

# --- GUARD 1b: aos-spawn isolation abort -------------------------------------

# A fake tmux that reports AOS_FAKE_PANE_PATH as the post-`treehouse get` pane cwd
...
set -u
case "$*" in
  *"#{pane_current_path}"*) printf '%s\n' "${AOS_FAKE_PANE_PATH:-}"; exit 0 ;;
esac
case "${1:-}" in
  display-message) printf 'maestro\n'; exit 0 ;;
  list-windows) exit 0 ;;
...
run_spawn() {
  ...
  AOS_ROOT_OVERRIDE='' AOS_HOME="$home" \
    AOS_STATE_OVERRIDE="$home/state" AOS_DATA_OVERRIDE="$home/data" \
    AOS_PROJECTS_OVERRIDE="$home/projects" AOS_CONFIG_OVERRIDE="$home/config" \
    AOS_SPAWN_NO_GUARD=1 AOS_FAKE_PANE_PATH="$pane" TMUX="fake,1,0" \
    PATH="$fakebin:$PATH" \
    "$ROOT/bin/aos-spawn.sh" "$id" "$proj" codex 2>&1
}
...
test_spawn_isolation_abort() {
  ...
  pass "aos-spawn: aborts unless the resolved worktree is a genuine, isolated worktree"
}
