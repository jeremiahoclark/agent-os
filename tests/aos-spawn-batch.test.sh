#!/usr/bin/env bash
# Behavior tests for aos-spawn.sh batch dispatch (`id=repo` pairs).
#
...
# the tests create no windows or worktrees. AOS_SPAWN_NO_GUARD=1 keeps them off the
...
SPAWN="$ROOT/bin/aos-spawn.sh"
TMP_ROOT=$(fm_test_tmproot aos-spawn-batch)

# Clear ambient maestro overrides so the behavior test owns its environment.
run_spawn() {
  AOS_ROOT_OVERRIDE='' \
    AOS_HOME='' \
    AOS_STATE_OVERRIDE='' \
    AOS_DATA_OVERRIDE='' \
    AOS_PROJECTS_OVERRIDE='' \
    AOS_CONFIG_OVERRIDE='' \
...
# A projects/ path is resolved through the maestro home, never the caller cwd,
...
test_projects_path_scoping() {
  ...
  while IFS='|' read -r label use_override id; do
    ...
    if [ "$use_override" = yes ]; then
      out=$(AOS_ROOT_OVERRIDE='' AOS_STATE_OVERRIDE='' AOS_DATA_OVERRIDE='' AOS_CONFIG_OVERRIDE='' \
        AOS_HOME="$home" AOS_PROJECTS_OVERRIDE="$projects" AOS_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    else
      mkdir -p "$home/projects/alpha"
      out=$(AOS_ROOT_OVERRIDE='' AOS_STATE_OVERRIDE='' AOS_DATA_OVERRIDE='' AOS_PROJECTS_OVERRIDE='' AOS_CONFIG_OVERRIDE='' \
        AOS_HOME="$home" AOS_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    fi
    status=$?
...
AOS_HOME scopes projects/|no|nope-home-z7
AOS_PROJECTS_OVERRIDE scopes projects/|yes|nope-override-z8
ROWS
  pass "projects/ paths are scoped through the maestro home for single-task spawn"
}
