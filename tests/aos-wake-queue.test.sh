#!/usr/bin/env bash
# tests/aos-wake-queue.test.sh - wake-queue losslessness (the queue safety matrix):
...
# lives in aos-watcher-lock.test.sh; daemon classification/injection in
# aos-daemon.test.sh.
set -u
...
TMP_ROOT=$(fm_test_tmproot aos-wake-tests)
...
test_concurrent_append_and_drain() {
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pids="$pids $!"
  for pid in $pids; do
    wait "$pid" || fail "concurrent append/drain subprocess failed"
  done
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" || fail "final drain failed"
  cat "$out1" "$out2" > "$all"
...
test_signal_catchup_without_running_watcher() {
  ...
  # Use an owner-relevant verb so the wake is surfaced and the catch-up path is
  # tested.
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after first signal failed"
...
test_stale_enqueue_before_suppressor() {
  ...
  # A stale pane sitting on an owner-relevant (terminal) status is actionable, so
  ...
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_WINDOW="$window" AOS_FAKE_TMUX_CAPTURE="$capture_file" AOS_STATE_OVERRIDE="$state" AOS_POLL=1 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after stale wake failed"
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  # Non-terminal status (no owner-relevant verb); prime .seen-* so the per-poll
  ...
  # fake aos-subagent-state.sh the watcher reads via AOS_SUBAGENT_STATE_BIN.)
  ...
  PATH="$fakebin:$PATH" AOS_FAKE_TMUX_WINDOW="$window" AOS_FAKE_TMUX_CAPTURE="$capture_file" \
    AOS_STATE_OVERRIDE="$state" AOS_SUBAGENT_STATE_BIN="$fakebin/aos-subagent-state.sh" \
    AOS_STALE_ESCALATE_SECS=999 AOS_POLL=1 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=999999 AOS_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after the immediate stale wake failed"
...
SH
  chmod +x "$check_file"
  PATH="$fakebin:$PATH" AOS_STATE_OVERRIDE="$state" AOS_POLL=1 AOS_SIGNAL_GRACE=1 AOS_CHECK_INTERVAL=0 AOS_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after check wake failed"
...
test_atomic_double_drain() {
  ...
  append_wake "$state" stale 's:aos-task' 'stale: s:aos-task' || fail "stale append failed"
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pid1=$!
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" &
  pid2=$!
  ...
  leftover=$(AOS_STATE_OVERRIDE="$state" "$DRAIN" | awk 'NF { count++ } END { print count + 0 }')
...
test_drain_dedupes_obvious_duplicates() {
  ...
  AOS_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "dedupe drain failed"
...
# watcher liveness via aos-guard.sh: a lapsed re-arm chain then surfaces even on a
...
test_drain_asserts_watcher_liveness() {
  ...
  state="$dir/state"
  err="$dir/drain.err"
  printf 'window=test:aos-x\nkind=ship\n' > "$state/x.meta"
  AOS_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2> "$err" || fail "drain failed while asserting liveness"
  ...
  AOS_STATE_OVERRIDE="$state" AOS_GUARD_GRACE=300 "$DRAIN" >/dev/null 2> "$err" || fail "drain failed with a fresh beacon"
