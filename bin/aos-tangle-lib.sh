...
# Shared worktree-tangle guard for the maestro-on-itself case.
# Usage: . bin/aos-tangle-lib.sh
#
# Maestro is a treehouse-pooled git repo of itself: subagent worktrees and
# secondary_subagent homes are all linked `git worktree`s of the same repo, while the
# PRIMARY checkout (the repo root maestro operates from) is a normal checkout
# on a real branch - normally the default branch, main. The "worktree tangle"
# failure mode is a subagent spawned to work on maestro ITSELF branching and
...
# which is how every linked worktree and secondary_subagent home legitimately sits on the
...
# echo nothing and return 1. Detached HEAD is how linked worktrees and secondary_subagent
