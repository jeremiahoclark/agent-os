...
# secondary_subagent home, kill the tmux window, clear volatile state, refresh/prune
...
# branch (maestro performs that merge on the owner's approval) as a fallback
# for the common case where there is no remote at all.
# Scout tasks (kind=scout in meta) carve out of that check: their worktree is
# declared scratch and the report at data/<task-id>/report.md is the work
# product - teardown proceeds once the report exists, and refuses without it.
# Secondary_subagents (kind=secondary_subagent in meta) are retired explicitly. Normal
# teardown refuses while their home has in-flight subagent meta files; --force
...
# Usage: aos-teardown.sh <task-id> [--force]
#   --force skips ordinary-task dirty and landed-work checks, skips scout report
#   checks, and discards secondary_subagent child work for kind=secondary_subagent. Only use it
#   when the owner has explicitly said to discard the work.
set -eu
...
SECONDARY_SUBAGENT_REG="$DATA/secondary_subagents.md"
...
maestro_home_has_treehouse_slot() {
  local home=$1
  worktree_registered_for_project "$AOS_ROOT" "$home"
}
...
validate_removal_target() {
  ...
  [ -e "$target" ] || return 0
  abs_target=$(removal_target_abs_path "$target")
  if abs_home=$(cd "$AOS_HOME" 2>/dev/null && pwd -P); then
    :
  else
    abs_home=
  fi
  abs_root=$(cd "$AOS_ROOT" && pwd -P)
  case "$abs_target" in
    ''|/) echo "REFUSED: unsafe $label removal target $target" >&2; return 1 ;;
  esac
  if [ -n "$abs_home" ] && [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label removal target $target is the active maestro home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label removal target $target is the maestro repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the active maestro home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the maestro repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the active maestro home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the maestro repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
validate_maestro_operational_dirs_for_removal() {
  ...
  for name in data state config projects; do
    ...
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      echo "REFUSED: unsafe $label $name directory $dir resolves outside the secondary_subagent home" >&2
      return 1
    fi
    if [ -d "$dir" ]; then
...
validate_child_worktree_for_removal() {
  ...
  if abs_home=$(cd "$AOS_HOME" 2>/dev/null && pwd -P); then
    if path_is_ancestor_of "$abs_home" "$abs_target"; then
      echo "REFUSED: unsafe child worktree removal target $target is inside the active maestro home" >&2
      return 1
    fi
  fi
  abs_root=$(cd "$AOS_ROOT" && pwd -P)
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe child worktree removal target $target is inside the maestro repo" >&2
    return 1
  fi
...
validate_maestro_home_for_removal() {
  ...
  if [ ! -f "$abs_home_path/$SUB_HOME_MARKER" ]; then
    echo "REFUSED: unsafe $label removal target $home is not a seeded secondary_subagent home" >&2
    return 1
  fi
  if [ -n "$expected_id" ]; then
    marker_id=$(cat "$abs_home_path/$SUB_HOME_MARKER" 2>/dev/null || true)
    if [ "$marker_id" != "$expected_id" ]; then
      echo "REFUSED: unsafe $label removal target $home is marked for secondary_subagent ${marker_id:-unknown}, expected $expected_id" >&2
      return 1
    fi
  fi
  validate_maestro_operational_dirs_for_removal "$abs_home_path" "$label" || return 1
  conflict=$(registered_descendant_home_for_removal "$SECONDARY_SUBAGENT_REG" "$abs_home_path" || true)
  if [ -z "$conflict" ]; then
    conflict=$(registered_descendant_home_for_removal "$abs_home_path/data/secondary_subagents.md" "$abs_home_path" || true)
  fi
  if [ -n "$conflict" ]; then
    IFS=$'\t' read -r child_id child_home <<EOF
$conflict
EOF
    echo "REFUSED: unsafe $label removal target $home contains registered secondary_subagent home $child_home for $child_id" >&2
    return 1
  fi
  printf '%s\n' "$abs_home_path"
}
...
remove_maestro_home() {
  ...
  [ -n "$home" ] || return 0
  [ -e "$home" ] || return 0
  abs_home_path=$(validate_maestro_home_for_removal "$home" "$label" "$expected_id") || return 1
  [ -n "$abs_home_path" ] || return 0
  if maestro_home_has_treehouse_slot "$abs_home_path"; then
    command -v treehouse >/dev/null 2>&1 || {
      echo "error: treehouse command not found; cannot return $label $abs_home_path" >&2
      return 1
    }
    ( cd "$AOS_ROOT" && treehouse return --force "$abs_home_path" ) || {
...
validate_maestro_home_children_removal() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondary_subagent ]; then
      child_home=$(meta_value "$child_meta" home)
      [ -n "$child_home" ] || child_home=$child_wt
      validate_maestro_home_for_removal "$child_home" "child maestro home" "$child_id" >/dev/null || return 1
      validate_maestro_home_children_removal "$child_home" || return 1
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondary_subagent ]; then
      ...
      if [ -n "$child_home" ] && [ -d "$child_home" ]; then
        cleanup_maestro_home_children "$child_home"
        remove_maestro_home "$child_home" "child maestro home" "$child_id"
      fi
    elif [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
      validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
      rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/aos-turn-end.js"
...
remove_secondary_subagent_registry_entry() {
  local id=$1 tmp
  [ -f "$SECONDARY_SUBAGENT_REG" ] || return 0
  tmp="$SECONDARY_SUBAGENT_REG.tmp.$$"
  grep -vE "^- $id( |$)" "$SECONDARY_SUBAGENT_REG" > "$tmp" || true
  mv "$tmp" "$SECONDARY_SUBAGENT_REG"
}

if [ "$KIND" = secondary_subagent ]; then
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
  validate_maestro_home_for_removal "$HOME_PATH" "secondary_subagent home" "$ID" >/dev/null || exit 1
  if [ "$FORCE" = "--force" ]; then
    validate_maestro_home_children_removal "$HOME_PATH" || exit 1
  fi
fi

if [ "$KIND" = secondary_subagent ] && [ "$FORCE" != "--force" ]; then
  SUB_STATE="$HOME_PATH/state"
  if [ -d "$SUB_STATE" ]; then
    for child_meta in "$SUB_STATE"/*.meta; do
      [ -e "$child_meta" ] || continue
      echo "REFUSED: secondary_subagent $ID still has in-flight work in $SUB_STATE." >&2
...
if [ "$KIND" = secondary_subagent ] && [ "$FORCE" = "--force" ]; then
  cleanup_maestro_home_children "$HOME_PATH"
fi
...
if [ -d "$WT" ] && [ "$FORCE" != "--force" ]; then
  if [ "$KIND" = secondary_subagent ]; then
    ...
    if [ ! -f "$REPORT" ]; then
      ...
      echo "The report is the work product. Have the subagent write it (or get the owner's explicit OK to discard, then --force)." >&2
      exit 1
    fi
  else
    # The aos-spawn hook file is ours, never work product; ignore it in the dirty check.
    ...
    if [ -n "$unpushed" ] && [ "$MODE" = local-only ]; then
      ...
      # into the local default branch (maestro does that merge on the owner's
      ...
      if [ -n "$dirty" ] || [ -n "$unmerged" ]; then
        ...
        echo "Merge the branch into local $DEFAULT first (bin/aos-merge-local.sh after the owner approves), or push to a fork/remote, or get the owner's explicit OK to discard, then --force." >&2
        exit 1
      fi
    elif [ -n "$dirty" ]; then
      ...
      echo "Commit them (or get the owner's explicit OK to discard, then --force)." >&2
      exit 1
      ...
      if ! work_is_landed "$branch"; then
        ...
        echo "Push the branch, land its PR, or get the owner's explicit OK to discard, then --force." >&2
        exit 1
      fi
    fi
  fi
...
if [ -d "$WT" ] && [ "$KIND" != secondary_subagent ]; then
  branch=$(git -C "$WT" rev-parse --abbrev-ref HEAD 2>/dev/null || echo HEAD)
  if [ "$branch" != "HEAD" ]; then
  ...
  rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/aos-turn-end.js"
...
if [ "$KIND" = secondary_subagent ]; then
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
  remove_maestro_home "$HOME_PATH" "secondary_subagent home" "$ID"
  remove_secondary_subagent_registry_entry "$ID"
fi
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.check.sh" "$STATE/$ID.meta" "$STATE/$ID.pi-ext.ts"
if [ "$KIND" != scout ] && [ "$KIND" != secondary_subagent ] && [ "$MODE" != local-only ]; then
  "$AOS_ROOT/bin/aos-fleet-sync.sh" "$PROJ" || true
fi
