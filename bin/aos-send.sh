#!/usr/bin/env bash
# Send one line of literal text to a subagent window, then Enter.
# Usage: aos-send.sh <window> <text...>
#   <window> may be a bare maestro window name (aos-xyz), resolved through
#   this home's state/<id>.meta, or explicit session:window.
# Special keys instead of text: aos-send.sh <window> --key Escape   (or Enter, C-c, ...)
#
...
# all retries), aos-send exits NON-ZERO so the caller knows the steer did not land
...
# bin/aos-tmux-lib.sh. Tune with AOS_SEND_RETRIES (default 3) / AOS_SEND_SLEEP (0.4).
...
#
# From-maestro marker: when the resolved target is a bare `aos-<id>` whose meta
# records kind=secondary_subagent, the text is prefixed with the from-maestro marker
# (bin/aos-marker-lib.sh) so the secondary_subagent routes its reply via its status file
# or a status-pointed doc instead of stranding it in chat the main maestro
# never reads. A subagent/scout target, an explicit session:window escape-hatch
# target, and the --key path are never marked - their behavior is unchanged.
# After a successful text submit aos-send pauses AOS_SEND_SETTLE seconds (default 1,
...
# The pause is aos-send-only; the shared submit core (used by the away-mode daemon,
...
# Mark a from-maestro -> secondary_subagent request. Only a bare `aos-<id>` target,
# resolved through this home's meta and recording kind=secondary_subagent, is marked: the
# secondary_subagent then routes its reply via the status path (see aos-marker-lib.sh).
# An explicit session:window target (the escape hatch for windows outside this
# home) and any subagent/scout target are left unmarked, and so is the --key path.
MARK_PREFIX=""
case "$RAW_TARGET" in
  aos-*)
    meta="$STATE/${RAW_TARGET#aos-}.meta"
    if [ -f "$meta" ] && grep -q '^kind=secondary_subagent$' "$meta" 2>/dev/null; then
      MARK_PREFIX="$AOS_FROMFIRST_MARK"
    fi
    ;;
esac

# Resolve the target's harness from its meta (recorded by aos-spawn), used only to
# scope the codex `$<skill>` popup-settle below. A bare aos-<id> target carries
...
if [ "${1:-}" = "--key" ]; then
  ...
  retries=${AOS_SEND_RETRIES:-3}
  sleep_s=${AOS_SEND_SLEEP:-0.4}
  ...
  # subagent actually working instead of the stale idle pane. AOS_SEND_SETTLE=0
  # disables it. Scoped to this path only, never the shared submit core.
  [ "${AOS_SEND_SETTLE:-1}" = 0 ] || sleep "${AOS_SEND_SETTLE:-1}"
fi
