...
# worktree, so it cannot discard unlanded work; set AOS_FLEET_PRUNE=0 to disable it.
# Usage: aos-fleet-sync.sh [<project-dir>]
set -eu
...
usage() {
  echo "usage: aos-fleet-sync.sh [<project-dir>]" >&2
}
...
prune_gone_branches() {
  ...
  # net. Set AOS_FLEET_PRUNE=0 to skip pruning entirely.
  [ "${AOS_FLEET_PRUNE:-1}" != "0" ] || return 0
...
sync_project() {
  ...
  mode_line=$("$AOS_ROOT/bin/aos-project-mode.sh" "$label" 2>/dev/null || echo "no-mistakes off")
  mode=${mode_line%% *}
