#!/usr/bin/env bash
# Provision and route persistent secondary_subagent homes.
#
# Usage:
#   aos-home-seed.sh <id> <home|-> <project>...
#       Provision <home> as an isolated maestro home. If <home> is "-", acquire
#       a fresh maestro worktree via "treehouse get --lease", which durably
#       leases the worktree under the secondary_subagent <id> so the home survives with
#       no live process and is never recycled until the lease is released with
#       "treehouse return". Projects are cloned
#       from the active home into the secondary_subagent home's projects/ directory.
...
#       initialized, a .aos_secondary_subagent_home marker is written, and
#       data/secondary_subagents.md is updated.
...
#       Set AOS_SECONDARY_SUBAGENT_CHARTER='<charter>' to seed from inline charter text
#       when no filled charter brief exists. Set AOS_SECONDARY_SUBAGENT_SCOPE='<scope>'
#       to override the registry routing scope. Otherwise the registry summary
#       and scope are derived from the filled charter brief.
#   aos-home-seed.sh validate
#       Refuse duplicate ids, duplicate homes, and nested or overlapping homes in
#       data/secondary_subagents.md.
set -eu
...
REG="$DATA/secondary_subagents.md"
SUB_HOME_MARKER=".aos_secondary_subagent_home"

usage() {
  echo "usage: aos-home-seed.sh <id> <home|-> <project>..." >&2
  echo "       aos-home-seed.sh validate" >&2
}
...
registry_summary_for_brief() {
  local brief=$1
  if [ -n "${AOS_SECONDARY_SUBAGENT_CHARTER:-}" ]; then
    printf '%s\n' "$AOS_SECONDARY_SUBAGENT_CHARTER" | normalize_registry_text
  else
...
registry_scope_for_brief() {
  local brief=$1
  if [ -n "${AOS_SECONDARY_SUBAGENT_SCOPE:-}" ]; then
    printf '%s\n' "$AOS_SECONDARY_SUBAGENT_SCOPE" | normalize_registry_text
  else
...
validate_registry_home_text() {
  ...
  case "$home" in
    *';'*|*')'*|*$'\n'*)
      echo "error: secondary_subagent home path contains registry delimiters: $home" >&2
      return 1
      ;;
  esac
}
...
validate_registry() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/aos-maestros.XXXXXX")
  if [ -f "$REG" ]; then
    while IFS= read -r line; do
      case "$line" in
        "- "*)
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate secondary_subagent home assignment:\n%s\n' "$duplicate_homes" >&2
    return 1
  }
  duplicate_ids=$(awk -F '\t' '
    {
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate secondary_subagent id assignment:\n%s\n' "$duplicate_ids" >&2
    return 1
  }
  overlaps=$(awk -F '\t' '
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: overlapping secondary_subagent home assignment:\n%s\n' "$overlaps" >&2
    return 1
  }
  rm -f "$tmp"
  return 0
...
refuse_active_home_path() {
  ...
  abs_active_home=$(resolved_path "$AOS_HOME")
  abs_root=$(resolved_path "$AOS_ROOT")
  if [ "$abs_home" = "/" ]; then
    echo "error: secondary_subagent home cannot be the filesystem root: $home" >&2
    return 1
  fi
  if [ "$abs_home" = "$abs_active_home" ]; then
    echo "error: secondary_subagent home cannot be the active maestro home: $home" >&2
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    echo "error: secondary_subagent home cannot be the maestro repo: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    echo "error: secondary_subagent home cannot be inside the active maestro home: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    echo "error: secondary_subagent home cannot be inside the maestro repo: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    echo "error: secondary_subagent home cannot be an ancestor of the active maestro home: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    echo "error: secondary_subagent home cannot be an ancestor of the maestro repo: $home" >&2
    return 1
  fi
}
...
validate_operational_dir() {
  ...
  dir="$home/$name"
  if [ -L "$dir" ] && [ ! -e "$dir" ]; then
    echo "error: secondary_subagent $name directory must resolve inside the secondary_subagent home: $dir" >&2
    return 1
  fi
  abs_home=$(resolved_path "$home")
  ...
  if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
    echo "error: secondary_subagent $name directory cannot be inside the active maestro home: $dir" >&2
    return 1
  fi
  if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
    echo "error: secondary_subagent $name directory cannot be inside the maestro repo: $dir" >&2
    return 1
  fi
}
...
validate_seed_leaf_files() {
  ...
  for label in "data/projects.md" "data/charter.md" "$SUB_HOME_MARKER"; do
    path="$home/$label"
    if [ -L "$path" ]; then
      echo "error: secondary_subagent leaf file must not be a symlink: $path" >&2
      return 1
    fi
    [ -e "$path" ] || continue
    ...
    case "$abs_path" in
      "$abs_home"/*) ;;
      *)
        echo "error: secondary_subagent leaf file must resolve inside the secondary_subagent home: $path" >&2
        return 1
        ;;
    esac
  done
...
validate_project_destination() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "error: secondary_subagent projects directory must resolve inside the secondary_subagent home: $projects_dir" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_dst"; then
    echo "error: seeded project $project destination must resolve inside the secondary_subagent projects directory: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the active maestro home: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the maestro repo: $dst" >&2
    return 1
  fi
  printf '%s\n' "$abs_dst"
}
...
acquire_treehouse_home() {
  local id=$1 home
  # Durably lease a maestro worktree from the pool. The lease persists with no
  ...
  home=$(cd "$AOS_ROOT" && treehouse get --lease --lease-holder "$id") || {
    echo "error: treehouse get --lease failed to lease a maestro home" >&2
    return 1
  }
  [ -n "$home" ] || { echo "error: treehouse get --lease did not report a maestro home" >&2; return 1; }
  printf '%s\n' "$home"
}
...
verify_maestro_home() {
  local home=$1
  refuse_active_home_path "$home" || return 1
  [ -f "$home/AGENTS.md" ] || { echo "error: $home is not a maestro home (missing AGENTS.md)" >&2; return 1; }
  [ -d "$home/bin" ] || { echo "error: $home is not a maestro home (missing bin/)" >&2; return 1; }
...
validate_home_assignment() {
  ...
  if [ -f "$home/$SUB_HOME_MARKER" ]; then
    ...
    if [ "$marker_id" != "$id" ]; then
      echo "error: secondary_subagent home $home is already marked for ${marker_id:-unknown}" >&2
      return 1
    fi
  fi
  id_conflict=$(registry_id_conflict_for_assignment "$id" "$home" || true)
  if [ -n "$id_conflict" ]; then
    echo "error: secondary_subagent id $id is already registered to home $id_conflict; retire it before assigning $home" >&2
    return 1
  fi
...
EOF
  if [ "$conflict_type" = exact ]; then
    echo "error: secondary_subagent home $home is already registered to $owner" >&2
    return 1
  fi
  echo "error: secondary_subagent home $home overlaps registered secondary_subagent home $registered_home for $owner" >&2
  return 1
}
...
clone_project() {
  ...
  read -r mode _ <<EOF
$(AOS_HOME="$AOS_HOME" AOS_DATA_OVERRIDE="$DATA" "$AOS_ROOT/bin/aos-project-mode.sh" "$project")
EOF
  if [ "$mode" = local-only ]; then
    echo "error: project $project is local-only; secondary_subagent routes support only no-mistakes and direct-PR projects" >&2
    return 1
  fi
  if [ -e "$dst" ]; then
...
seed_rollback_target() {
  ...
  abs_home=$(resolved_path "$AOS_HOME")
  abs_root=$(resolved_path "$AOS_ROOT")
  if [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the active maestro home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the maestro repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the active maestro home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the maestro repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the active maestro home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the maestro repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_return_treehouse_home() {
  ...
  ( cd "$AOS_ROOT" && treehouse return --force "$abs_home" >/dev/null ) || {
...
seed_project_rollback_target() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "REFUSED: unsafe created project rollback target $target has projects directory outside the secondary_subagent home" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_target"; then
    echo "REFUSED: unsafe created project rollback target $target is outside the secondary_subagent projects directory" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_rollback() {
  ...
  if [ -n "${SEED_BACKUP_DIR:-}" ]; then
    restore_seed_file "$SEED_PARENT_REG_EXISTED" "$SEED_BACKUP_DIR/parent_secondary_subagents.md" "$REG"
...
project_mode_in_home() {
  ...
  read -r mode _ <<EOF
$(AOS_ROOT_OVERRIDE='' AOS_STATE_OVERRIDE='' AOS_DATA_OVERRIDE='' AOS_PROJECTS_OVERRIDE='' AOS_CONFIG_OVERRIDE='' AOS_HOME="$home" "$AOS_ROOT/bin/aos-project-mode.sh" "$project")
EOF
  printf '%s\n' "$mode"
}
...
seed_home() {
  ...
  shift 2
  [ $# -gt 0 ] || { echo "error: secondary_subagent needs at least one project" >&2; return 1; }
  ...
  SEED_HOME_ACQUIRED=0
  SEED_HOME_BACKED_UP=0
  SEED_BACKUP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/aos-home-seed.XXXXXX")
  ...
  if [ -f "$REG" ]; then
    ...
    cp "$REG" "$SEED_BACKUP_DIR/parent_secondary_subagents.md"
  fi
  ...
  if [ ! -f "$SEED_PARENT_BRIEF" ]; then
    [ -n "${AOS_SECONDARY_SUBAGENT_CHARTER:-}" ] || {
      echo "error: no filled secondary_subagent charter brief at $SEED_PARENT_BRIEF; set AOS_SECONDARY_SUBAGENT_CHARTER or scaffold one and replace {TASK}" >&2
      return 1
    }
    [ -d "$DATA/$id" ] || SEED_PARENT_BRIEF_DIR_CREATED=1
    "$AOS_ROOT/bin/aos-brief.sh" "$id" --secondary_subagent "$@"
    SEED_PARENT_BRIEF_CREATED=1
  fi
  if grep -F '{TASK}' "$SEED_PARENT_BRIEF" >/dev/null 2>&1; then
    echo "error: secondary_subagent charter brief at $SEED_PARENT_BRIEF still contains {TASK}; fill it before seeding" >&2
    return 1
  fi
  charter_summary=$(registry_summary_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_summary" ] || {
    echo "error: secondary_subagent charter brief at $SEED_PARENT_BRIEF has an empty Charter section; fill it before seeding" >&2
    return 1
  }
  charter_scope=$(registry_scope_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_scope" ] || {
    echo "error: secondary_subagent charter brief at $SEED_PARENT_BRIEF has an empty Routing scope section; fill it before seeding" >&2
    return 1
  }
