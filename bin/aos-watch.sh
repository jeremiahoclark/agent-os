...
#   heartbeat              fleet-scan backstop found an unsurfaced owner-relevant
#                          status, unless afk is active
# For normal supervision, re-arm after each printed reason by running
# bin/aos-watch-arm.sh through the harness's tracked background mechanism. Direct
...
# Shared wake classifier (owner-relevant verbs + signal/stale/heartbeat
# predicates), the SAME library the away-mode daemon uses, so the triage policy
# has one definition.
# shellcheck source=bin/aos-classify-lib.sh
. "$SCRIPT_DIR/aos-classify-lib.sh"

WATCH_LOCK="$STATE/.watch.lock"
WATCH_PATH="$SCRIPT_DIR/aos-watch.sh"
WATCHER_STALE_GRACE=${AOS_WATCHER_STALE_GRACE:-${AOS_GUARD_GRACE:-300}}
  ...
  if [ -n "${AOS_LOCK_HELD_PID:-}" ]; then
    if [ -e "$BEAT" ]; then
      ...
      if [ "$beat_age" -ge "$WATCHER_STALE_GRACE" ]; then
        echo "watcher: lock held by live pid $AOS_LOCK_HELD_PID but heartbeat is stale for ${beat_age}s (>${WATCHER_STALE_GRACE}s); inspect or stop that watcher before re-arming." >&2
        exit 1
      fi
    elif [ "$(fm_path_age "$WATCH_LOCK")" -ge "$WATCHER_STALE_GRACE" ]; then
      echo "watcher: lock held by live pid $AOS_LOCK_HELD_PID but no heartbeat exists; inspect or stop that watcher before re-arming." >&2
      exit 1
    fi
    echo "watcher: already running pid $AOS_LOCK_HELD_PID"
  else
...
WATCHER_PID=${BASHPID:-$$}
printf '%s\n' "$AOS_HOME" > "$WATCH_LOCK/aos-home" || true
...
POLL=${AOS_POLL:-15}                   # seconds between cycles
HEARTBEAT=${AOS_HEARTBEAT:-600}        # base seconds between heartbeat scans
HEARTBEAT_MAX=${AOS_HEARTBEAT_MAX:-7200}  # heartbeat backoff cap
CHECK_INTERVAL=${AOS_CHECK_INTERVAL:-300}  # seconds between *.check.sh sweeps
CHECK_TIMEOUT=${AOS_CHECK_TIMEOUT:-30}     # seconds allowed per *.check.sh
SIGNAL_GRACE=${AOS_SIGNAL_GRACE:-30}   # seconds to linger after a signal so trailing
...
# (aos-classify-lib.sh) backs the away-mode daemon; while state/.afk exists the
# daemon owns triage, so this watcher reverts to one-shot (enqueue + exit on every
# wake) and never double-triages - and never runs the costly provably-working read.
STALE_ESCALATE_SECS=${AOS_STALE_ESCALATE_SECS:-240}  # idle secs before a non-terminal stale escalates as a possible wedge
TRIAGE_LOG="$STATE/.watch-triage.log"
TRIAGE_LOG_MAX_BYTES=${AOS_WATCH_TRIAGE_LOG_MAX_BYTES:-262144}
...
# owner-relevant status line it SURFACED (woke maestro for) in
...
# fleet-scan can tell apart an owner-relevant status that already woke maestro
...
# Record a status file's owner-relevant last line as surfaced (no-op for a
# non-owner-relevant or empty status). Call AFTER the wake is enqueued, so the
...
mark_surfaced() {  # <status-file>
  ...
  [ -n "$last" ] || return 0
  status_is_owner_relevant "$last" || return 0
  printf '%s' "$last" > "$(_hb_surfaced_path "$task")"
}

# Mark every current owner-relevant status as surfaced. Called after the
# heartbeat backstop enqueues its wake, so the same statuses are not re-surfaced
# by the next heartbeat.
mark_all_owner_relevant_surfaced() {
  local f task last
...
# any owner-relevant status has NOT already been surfaced to maestro (its
...
# owner-relevant signal/stale already marks itself surfaced when it wakes
# maestro, this normally finds nothing and the heartbeat is absorbed; it
# surfaces only an owner-relevant status the per-wake path absorbed by mistake -
...
while :; do
  ...
  # Liveness beacon for aos-guard.sh: a fresh mtime here means a watcher is
  # alive. Supervision scripts warn when this goes stale with tasks in flight.
  touch "$STATE/.last-watcher-beat"

  # Slow per-task checks (maestro writes these, e.g. a merged-PR poll).
  # Time-based via .last-check mtime so the cadence survives watcher restarts.
  # Evaluated BEFORE the signal scan: wake() exits the cycle, so a check placed
  # after the signal scan would be starved whenever a chatty sibling subagent
  ...
  # classifying: a subagent's final status write and the same turn's turn-end
  # hook land seconds apart, and reporting them as separate actionable wakes
  # costs a full maestro turn each. The re-scan also picks up a newer
...
EOF
  ...
  # signature means the subagent finished, is waiting, or is wedged. Each distinct
  # stale hash is surfaced, absorbed, or timed toward escalation once (.stale-*
  # remembers the hash already classified).
  while IFS= read -r w; do
    # A secondary_subagent idling on its own watcher is healthy. Its parent supervises
    # it through status writes and heartbeats, not pane-idle staleness.
    [ "$(window_kind "$w")" = secondary_subagent ] && continue
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && ! printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 | grep -qiE "$BUSY_REGEX"; then
        ...
        # maestro. Detection itself is unchanged from above.
        if afk_present; then
          ...
          if [ "$(cat "$sf" 2>/dev/null || true)" != "$h" ]; then
            if subagent_is_provably_working "$(window_to_task "$w")"; then
              printf '%s' "$h" > "$sf"
              date +%s > "$ssf"
  ...
  if [ "$(age_of "$STATE/.last-heartbeat")" -ge "$hb" ]; then
    ...
    # turns up an owner-relevant status the per-wake path missed. Absorb the
    ...
    if afk_present; then
      ...
      # Backstop: an owner-relevant status the per-wake path absorbed by mistake.
      # Enqueue first, then mark every owner-relevant status surfaced so the next
      ...
      triage_log "absorbed heartbeat (no owner-relevant change)"
    fi
  fi
