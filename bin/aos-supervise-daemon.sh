#!/usr/bin/env bash
# aos-supervise-daemon.sh — presence-gated sub-supervisor (closes #27's P2).
#
# Wraps bin/aos-watch.sh: runs it as a child, classifies each wake reason, and
# either SELF-HANDLES the routine majority in bash (no maestro turn) or
# ESCALATES a batched, distilled digest to the supervisor pane on
# owner-relevant events only. This is the token-efficient replacement for the
# prior always-inject daemon: routine signal/stale/heartbeat wakes cost zero
# maestro context; only done/needs-decision/blocked/failed/persistent-wedge/
...
# user message clears it and maestro resumes full responsiveness.
# When afk is off, normal aos-watch.sh always-on triage is the active mechanism.
...
# AOS_INJECT_MARK (ASCII unit separator, 0x1f) — a byte a human would never type
# at the start of a message. Maestro's contract: a message that starts with
# the marker is an internal escalation (stay afk); a message without it means
# the owner is back (exit afk, flush catch-up, resume per-wake responsiveness).
...
#     crash/restart/missed injection is recovered on the next aos-wake-drain.sh.
...
#     subagent is therefore detected within STALE_ESCALATE_SECS + a tick, never
#     lost. Subagents are autonomous, so a delayed stale response does not stall
#     a healthy subagent's own progress.
#     Buffered escalation delivery also has a max-defer alarm: if a digest stays
#     undelivered past AOS_MAX_DEFER_SECS, the daemon retries a normal flush and
#     writes state/.subsuper-inject-wedged if submit still cannot be confirmed.
#   - Cheap heartbeat catch-all: every HEARTBEAT_SCAN_SECS the daemon greps all
#     state/*.status for an owner-relevant line the per-wake classifier might
#     have missed (e.g. a status verb outside OWNER_RE) and escalates it.
#
...
#
# Usage: aos-supervise-daemon.sh
#          Long-lived background loop. Normally started by the /afk skill, which
#          sets state/.afk first. Env knobs:
#          AOS_SUPERVISOR_TARGET     supervisor tmux target (override; otherwise
#                                   auto-discovered from TMUX_PANE, then
#                                   maestro:0 fallback)
#          AOS_INJECT_SKIP           |-prefixes force-self-handle bypassing
#                                   classification (default "heartbeat"); empty
#                                   disables. Use sparingly: it overrides the
#                                   owner-relevant escalation for matching
#                                   kinds.
#          AOS_STALE_ESCALATE_SECS   idle seconds before a stale pane escalates
#                                   as a possible wedge (default 240)
#          AOS_ESCALATE_BATCH_SECS   buffer window for batched escalation
#                                   digests; 0 = flush immediately (default 90)
#          AOS_HEARTBEAT_SCAN_SECS   cadence for the catch-all status scan
#                                   (default 300)
#          AOS_HOUSEKEEPING_TICK     seconds between housekeeping passes while
#                                   the watcher is mid-cycle (default 15)
#          AOS_BUSY_REGEX            OR-ed busy signatures (mirrors aos-watch.sh)
#          AOS_COMPOSER_IDLE_RE      empty-composer regex applied after dim-ghost
#                                   and structural border stripping (default:
#                                   bare prompt glyphs plus busy footers)
#          AOS_MAX_DEFER_SECS        max seconds a buffered escalation may sit
#                                   undelivered before one normal flush attempt;
#                                   if that cannot confirm a submit, a wedge
#                                   alarm fires (default 300; 0 disables)
#          AOS_INJECT_CONFIRM_RETRIES Enter-retry attempts on a swallowed Enter
#                                   (default 3); the digest is typed once, only
#                                   Enter is retried. Composer-empty detection is
#                                   structural and style-aware (bin/aos-tmux-lib.sh):
...
#          AOS_INJECT_CONFIRM_SLEEP  seconds between daemon submit checks
#                                   (default 0.5)
#          AOS_LOG_MAX_BYTES / AOS_LOG_KEEP_LINES / AOS_CRASH_*  log + crash guards
#          AOS_STATE_OVERRIDE        alternate state dir (testing)
...
#          lock. A crashing aos-watch.sh is logged and restarted, never killing
#          the daemon; a tight crash-restart spin is detected and backed off.
set -u

AOS_DAEMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
AOS_ROOT="${AOS_ROOT_OVERRIDE:-$(cd "$AOS_DAEMON_DIR/.." && pwd)}"
...
# shellcheck source=bin/aos-tmux-lib.sh
. "$AOS_DAEMON_DIR/aos-tmux-lib.sh"

# Shared wake classifier (last_status_line, status_is_owner_relevant,
# window_to_task, scan_owner_relevant_statuses). The SAME library backs the
# always-on watcher's triage, so the owner-relevant verb set and the
# classification predicates have exactly one definition.
# shellcheck source=bin/aos-classify-lib.sh
. "$AOS_DAEMON_DIR/aos-classify-lib.sh"

# --- tunables ---------------------------------------------------------------
AOS_SUPERVISOR_TARGET_DEFAULT="maestro:0"
...
# The owner-relevant verb set and the status classifiers (last_status_line,
# status_is_owner_relevant, window_to_task, scan_owner_relevant_statuses) now
# live in bin/aos-classify-lib.sh, shared with the always-on watcher.
# Busy footers + composer-empty detection now live in bin/aos-tmux-lib.sh
# (AOS_TMUX_BUSY_REGEX_DEFAULT / fm_tmux_composer_state); AOS_BUSY_REGEX still
...
# injection is prefixed with this byte; maestro treats a leading marker as an
# internal escalation (stay afk) and its absence as "owner is back" (exit afk).
# Portable across harnesses: it travels with the message text, independent of
# any harness-level typed-vs-injected distinction.
AOS_INJECT_MARK=$'\x1f'
AFK_FLAG_NAME=".afk"

# Resolve the effective state dir. AOS_STATE_OVERRIDE wins (testing); otherwise
# $AOS_HOME/state. Kept as a function so the pure
# classifiers can take an explicit state arg without depending on globals.
_state_root() { printf '%s' "${AOS_STATE_OVERRIDE:-$AOS_HOME/state}"; }

# --- portable stat (same trap as aos-watch.sh: no `stat -f || stat -c`) -------
...
# skill (enter) and by maestro on user return (exit). Durable: a plain file,
...
# should_exit_afk: encodes maestro's afk-exit contract as a testable function.
...
#   anything else           -> 0 (owner is back; exit afk)
# Bias toward exit: only the marker and an explicit /afk invocation keep afk
# alive. A false exit is self-correcting (the owner re-runs /afk).
...
# marker (a daemon escalation), 1 otherwise (a real user message). Maestro's
# afk-exit contract uses this: marker present -> stay afk; absent -> owner is
# back. Bias ambiguous cases toward exit (a false exit is self-correcting).
message_is_injection() {  # <message-text>
  local msg=$1
  [ -n "$msg" ] || return 1
  case "$msg" in
    "$AOS_INJECT_MARK"*) return 0 ;;
  esac
  return 1
}
...
# distilled content maestro relays to the owner or feeds back to classifiers.
strip_injection_marker() {  # <message-text>
  local msg=$1
  printf '%s' "${msg#"$AOS_INJECT_MARK"}"
}
...
#   1. AOS_SUPERVISOR_TARGET env (explicit override) — caller passes it in.
#   2. $TMUX_PANE — tmux sets this in every pane's environment; inherited by
#      the daemon when the /afk skill launches it from maestro's own pane.
#   3. maestro:0 — legacy fallback (may not resolve if the session is named
...
discover_supervisor_target() {
  if [ -n "${AOS_SUPERVISOR_TARGET:-}" ]; then
    printf '%s' "$AOS_SUPERVISOR_TARGET"
    return 0
  fi
  if [ -n "${TMUX_PANE:-}" ]; then
    printf '%s' "$TMUX_PANE"
  ...
  printf '%s' "$AOS_SUPERVISOR_TARGET_DEFAULT"
  return 1
}

# --- classification helpers (PURE: no side effects, testable) ---------------
# last_status_line, status_is_owner_relevant, window_to_task, and
# scan_owner_relevant_statuses come from bin/aos-classify-lib.sh (sourced above),
# the single classifier shared with bin/aos-watch.sh. The decision-string wrappers
...
# summary maestro would otherwise have to re-read.
...
classify_signal() {  # <reason-after-colon> <state>
  ...
  for f in $reason; do
    ...
    status_is_owner_relevant "$last" || continue
    rel=1
...
classify_stale() {  # <window> <state>
  ...
  if [ -n "$last" ] && status_is_owner_relevant "$last"; then
...
classify_check() {  # <full reason>  — check scripts print only when maestro should wake
  printf 'escalate|%s' "$1"
}
...
# Record the seen-status marker for an owner-relevant status line so the
...
# Mark every owner-relevant status line a per-wake classification escalated as
...
mark_escalated_seen() {  # <kind> <arg> <state>
  ...
  case "$kind" in
    ...
    stale)
      ...
      last=$(last_status_line "$state/$task.status")
      [ -n "$last" ] && status_is_owner_relevant "$last" \
        && mark_status_seen "$state" "$task" "$last" ;;
  esac
}

# Busy + composer-empty detection are the shared primitives in aos-tmux-lib.sh
# (one source of truth with aos-send.sh). These thin wrappers keep the daemon's
...
# an ERROR, drops a durable marker maestro/recovery can surface, and flashes
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  if [ "$(_file_age "$marker")" -lt "${AOS_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}" ]; then
    return 0
  fi
  ...
  target="${AOS_SUPERVISOR_TARGET:-$AOS_SUPERVISOR_TARGET_DEFAULT}"
...
#     owner-relevant line the per-wake classifier missed and escalate it.
housekeeping() {  # <state>
  local state=$1 now due f key task win marker age last max_defer oldest
  now=$(_now)

  # (1) batch flush
  if [ "${AOS_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ]; then
    escalate_flush "$state" || true
  else
    due=$(_oldest_line_age "$state/.subsuper-escalations")
    if [ "$due" -ge "${AOS_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" ]; then
      escalate_flush "$state" || true
    fi
  fi
  ...
  max_defer=${AOS_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    [ "$age" -ge "${AOS_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}" ] || continue
    # Reconstruct the window name from the key (best-effort: session is unknown,
    # so probe the live aos-* windows for one whose task matches).
    ...
    if pane_is_busy "$win"; then
      rm -f "$marker"   # subagent resumed: benign
    else
  ...
  # (3) heartbeat scan (catch-all for an owner-relevant status the per-wake
  #     classifier may have missed). Cheap: status files only, no tmux. The
  #     owner-relevant filtering is the shared classifier's
  #     scan_owner_relevant_statuses; the daemon layers its digest dedup on top.
  if [ "$(_file_age "$state/.subsuper-last-scan")" -ge "${AOS_HEARTBEAT_SCAN_SECS:-$HEARTBEAT_SCAN_SECS_DEFAULT}" ]; then
    ...
    while IFS="$(printf '\t')" read -r f task last; do
      ...
      mark_status_seen "$state" "$task" "$last"
    done < <(scan_owner_relevant_statuses "$state")
  fi
}

# Find a live aos-* window whose task id matches the given marker key.
window_for_task() {  # <task-key>
  local key=$1 w t
  for w in $(tmux list-windows -a -F '#{session_name}:#{window_name}' 2>/dev/null | grep ':aos-' || true); do
    t=$(window_to_task "$w")
...
inject_msg() {  # <message> [state]
  ...
  # daemon self-handles and stays quiet; maestro drives the normal always-on
  ...
  # them. Then prepend the sentinel marker — maestro's afk-exit contract
  # keys off its presence at the start of the message.
  msg=$(_collapse_newlines "$msg")
  msg="${AOS_INJECT_MARK}${msg}"
  target="${AOS_SUPERVISOR_TARGET:-$AOS_SUPERVISOR_TARGET_DEFAULT}"
  ...
  retries=${AOS_INJECT_CONFIRM_RETRIES:-$INJECT_CONFIRM_RETRIES_DEFAULT}
  sleep_s=${AOS_INJECT_CONFIRM_SLEEP:-$INJECT_CONFIRM_SLEEP_DEFAULT}
...
should_force_self() {  # <reason>
  local reason=$1 skip="${AOS_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}" prefix
  [ -n "$skip" ] || return 1
  local -a prefixes
...
handle_wake() {  # <reason> <state>
  ...
  if should_force_self "$reason"; then
    log "wake force-self (AOS_INJECT_SKIP): $reason"
    return
  fi
  case "$reason" in
  ...
  if [ "$action" = "escalate" ]; then
    ...
    mark_escalated_seen "$kind" "$arg" "$state"
    [ "${AOS_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ] && { escalate_flush "$state" || true; }
  else
...
trim_log() {
  ...
  [ "$sz" -ge "${AOS_LOG_MAX_BYTES:-$LOG_MAX_BYTES_DEFAULT}" ] || return 0
  tmp=$(mktemp "${TMPDIR:-/tmp}/aos-daemon-log.XXXXXX") || return 0
  tail -n "${AOS_LOG_KEEP_LINES:-$LOG_KEEP_LINES_DEFAULT}" "$LOG" >"$tmp" 2>/dev/null && mv -f "$tmp" "$LOG"
}

# ============================================================================
# Everything below runs only when the script is EXECUTED, not sourced. The pure
# classifiers above are sourceable for unit tests (tests/aos-daemon.test.sh).
# ============================================================================
...
fm_super_main() {
  ...
  # Export AOS_STATE_OVERRIDE so the lib resolves the same state dir.
  # shellcheck source=bin/aos-wake-lib.sh
  AOS_STATE_OVERRIDE="$STATE" . "$AOS_DAEMON_DIR/aos-wake-lib.sh"

  local WATCH="$AOS_DAEMON_DIR/aos-watch.sh"
  ...
  local INJECT_FAIL_SLEEP=${AOS_INJECT_FAIL_SLEEP:-$INJECT_FAIL_SLEEP_DEFAULT}
  local CRASH_THRESHOLD=${AOS_CRASH_THRESHOLD:-$CRASH_THRESHOLD_DEFAULT}
  local CRASH_WINDOW=${AOS_CRASH_WINDOW:-$CRASH_WINDOW_DEFAULT}
  local CRASH_BACKOFF=${AOS_CRASH_BACKOFF:-$CRASH_BACKOFF_DEFAULT}
  local CRASH_NORMAL_SLEEP=${AOS_CRASH_NORMAL_SLEEP:-$CRASH_NORMAL_SLEEP_DEFAULT}
  ...
  if ! fm_lock_try_acquire "$LOCK"; then
    if [ -n "${AOS_LOCK_HELD_PID:-}" ]; then
      echo "error: another aos-supervise-daemon is already running (pid $AOS_LOCK_HELD_PID, lock $LOCK held)" >&2
    else
      echo "error: another aos-supervise-daemon is already running (lock $LOCK held)" >&2
    fi
    exit 1
  fi
  echo "$$" > "$PIDFILE"

  # --- auto-discover the supervisor target (the pane running maestro) -----
  # Priority: AOS_SUPERVISOR_TARGET override > $TMUX_PANE (inherited from the
  # pane that launched the daemon, normally maestro's own) > maestro:0
  # fallback. Exporting the result into AOS_SUPERVISOR_TARGET makes inject_msg
  # (which reads that env var) use the discovered pane without an extra global.
  local discovered target_source
  target_source="AOS_SUPERVISOR_TARGET"
  if [ -z "${AOS_SUPERVISOR_TARGET:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      target_source="TMUX_PANE"
    else
      target_source="FALLBACK(maestro:0)"
    fi
  fi
  if discovered=$(discover_supervisor_target); then
    : # resolved cleanly
  else
    echo "warn: could not auto-discover supervisor pane (no AOS_SUPERVISOR_TARGET or TMUX_PANE); falling back to '$discovered' — verify this is maestro's pane" >&2
  fi
  AOS_SUPERVISOR_TARGET="$discovered"
  local TARGET="$AOS_SUPERVISOR_TARGET"
  ...
  if ! tmux display-message -p -t "$TARGET" '#{pane_id}' >/dev/null 2>&1; then
    echo "error: supervisor target '$TARGET' does not resolve to a tmux pane; set AOS_SUPERVISOR_TARGET" >&2
  ...
  log "daemon starting (pid $$); target=$TARGET; target_source=$target_source; afk=$afk_status; inject_skip='${AOS_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}'; stale_escalate=${AOS_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}s; batch=${AOS_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}s"
  ...
  start_watcher() {
    CUR_TMP=$(mktemp "${TMPDIR:-/tmp}/aos-watch.XXXXXX") || { log "error: mktemp failed; retrying in 5s"; sleep 5; return 1; }
  ...
  while true; do
    ...
    # has nowhere to go, and maestro itself is the consumer of escalations.
    ...
    # The watcher child runs on its own AOS_POLL cadence internally; we only need
    ...
    sleep 1
    if [ "$(_file_age "$STATE/.subsuper-last-housekeep")" -ge "${AOS_HOUSEKEEPING_TICK:-$HOUSEKEEPING_TICK_DEFAULT}" ]; then
