#!/usr/bin/env bash
# aos-tmux-lib.sh — shared tmux pane primitives for maestro.
#
...
# daemon (bin/aos-supervise-daemon.sh) and bin/aos-send.sh so the composer/submit
...
# idle pane as holding pending input and the daemon deferred injection / maestro
...
# surfaced (aos-peek and every human/LLM-facing path stay plain), and only the
...
#
# Per-harness override: AOS_COMPOSER_IDLE_RE matches an empty composer after
# dim-ghost and structural border stripping. AOS_BUSY_REGEX overrides the busy
# footer set (mirrors aos-watch.sh / the daemon).
#
# All functions are `set -u` and `set -e` safe (guarded tmux calls, explicit
# returns) so they can be sourced into either context.

# Busy footers per harness (mirror aos-watch.sh). claude/codex: "esc to
# interrupt"; opencode: "esc interrupt"; pi: "Working...".
AOS_TMUX_BUSY_REGEX_DEFAULT='esc (to )?interrupt|Working\.\.\.'
...
fm_tmux_composer_state() {  # <target> -> empty|pending|unknown
  ...
  if [ -n "${AOS_COMPOSER_IDLE_RE:-}" ] \
     && printf '%s' "$stripped" | grep -qiE "$AOS_COMPOSER_IDLE_RE"; then
  ...
  if printf '%s' "$stripped" | grep -qiE "${AOS_BUSY_REGEX:-$AOS_TMUX_BUSY_REGEX_DEFAULT}"; then
    printf 'empty'; return 0
  fi
...
# (an agent mid-turn). Scans a 40-line tail like aos-watch.sh.
fm_pane_is_busy() {  # <target>
  local win=$1 tail40
  tail40=$(tmux capture-pane -p -t "$win" -S -40 2>/dev/null) || return 1
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
    | grep -qiE "${AOS_BUSY_REGEX:-$AOS_TMUX_BUSY_REGEX_DEFAULT}"
}
...
#   - aos-send fails only on "pending" (lenient: a positively-confirmed swallow),
