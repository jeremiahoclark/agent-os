#!/usr/bin/env bash
# Shared config resolution for the X-mode connector client (aos-x-poll.sh and
# aos-x-reply.sh). X mode is opt-in: a user drops a non-empty AOSX_PAIRING_TOKEN
# into the maestro home's .env. AOSX_ENV_FILE can point direct client calls at
# another .env-style file, but bootstrap activation still checks $AOS_HOME/.env.
# Until then polling is a hard no-op; replies can still run in AOSX_DRY_RUN
...
#   fmx_load_config            - resolve AOSX_TOKEN, AOSX_RELAY, AOSX_DRY, AOSX_MAX,
#                                and AOSX_THREAD_MAX (env wins over .env)
...
# Callers must have AOS_HOME set before calling fmx_load_config.
...
# Resolve the X-mode settings into AOSX_TOKEN, AOSX_RELAY, AOSX_DRY, AOSX_MAX, and
# AOSX_THREAD_MAX. An explicit environment variable always wins over the .env
# file; the relay URL defaults to the production host so a normal user configures
# only the token. AOSX_RELAY has any trailing slash trimmed so callers can append
# "/connector/..." cleanly.
# AOSX_DRY is set to "1" when AOSX_DRY_RUN is a truthy value (anything other than
# unset/empty/0/false/no/off), and "" otherwise: preview mode, where the client
# composes a reply but records it instead of posting (see aos-x-reply.sh).
fmx_load_config() {
  local env_file="${AOSX_ENV_FILE:-$AOS_HOME/.env}" dry
  if [ -n "${AOSX_PAIRING_TOKEN+x}" ]; then
    AOSX_TOKEN=${AOSX_PAIRING_TOKEN-}
  else
    AOSX_TOKEN=$(fmx_env_get AOSX_PAIRING_TOKEN "$env_file")
  fi
  if [ -n "${AOSX_RELAY_URL+x}" ]; then
    AOSX_RELAY=${AOSX_RELAY_URL-}
  else
    AOSX_RELAY=$(fmx_env_get AOSX_RELAY_URL "$env_file")
  fi
  [ -n "$AOSX_RELAY" ] || AOSX_RELAY="https://mymaestro.io"
  AOSX_RELAY=${AOSX_RELAY%/}
  if [ -n "${AOSX_DRY_RUN+x}" ]; then
    dry=${AOSX_DRY_RUN-}
  else
    dry=$(fmx_env_get AOSX_DRY_RUN "$env_file")
  fi
  # shellcheck disable=SC2034 # AOSX_DRY is read by callers (aos-x-reply.sh) after sourcing.
  case "$(printf '%s' "$dry" | tr '[:upper:]' '[:lower:]')" in
    ''|0|false|no|off) AOSX_DRY="" ;;
    *) AOSX_DRY=1 ;;
  esac
  ...
  if [ -n "${AOSX_X_REPLY_MAX_CHARS+x}" ]; then maxraw=${AOSX_X_REPLY_MAX_CHARS-}; else maxraw=$(fmx_env_get AOSX_X_REPLY_MAX_CHARS "$env_file"); fi
  case "$maxraw" in ''|*[!0-9]*) maxraw=280 ;; esac
  [ "$maxraw" -ge 50 ] 2>/dev/null || maxraw=50
  # shellcheck disable=SC2034 # AOSX_MAX is read by callers (aos-x-reply.sh) after sourcing.
  AOSX_MAX=$maxraw
  if [ -n "${AOSX_X_THREAD_MAX+x}" ]; then threadraw=${AOSX_X_THREAD_MAX-}; else threadraw=$(fmx_env_get AOSX_X_THREAD_MAX "$env_file"); fi
  case "$threadraw" in ''|*[!0-9]*) threadraw=25 ;; esac
  [ "$threadraw" -ge 1 ] 2>/dev/null || threadraw=25
  # shellcheck disable=SC2034 # AOSX_THREAD_MAX is read by callers (aos-x-reply.sh) after sourcing.
  AOSX_THREAD_MAX=$threadraw
}
...
fmx_auth_header_file() {
  ...
  file=$(umask 077; mktemp "${TMPDIR:-/tmp}/aos-x-auth.XXXXXX") || return 1
  chmod 600 "$file" 2>/dev/null || { rm -f "$file"; return 1; }
  printf 'Authorization: Bearer %s\n' "$AOSX_TOKEN" > "$file" || { rm -f "$file"; return 1; }
  printf '%s\n' "$file"
}
...
# On the task's terminal completion maestro posts ONE follow-up reply to that
# request (within the window) and clears the link. These helpers own the
# read/write/clear so aos-x-link.sh and aos-x-followup.sh never hand-edit meta and
