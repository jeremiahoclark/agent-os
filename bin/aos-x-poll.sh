...
# via a non-empty AOSX_PAIRING_TOKEN (from the home's .env or the environment).
# This script is the body of the watcher check shim state/x-watch.check.sh, where
# the contract is "output => wake maestro, silence => keep sleeping", so the
...
# preserved for aosx-respond to handle follow-ups with continuity.
#
# Config (home .env, AOSX_ENV_FILE, or env): AOSX_PAIRING_TOKEN (required),
# AOSX_RELAY_URL (default https://mymaestro.io). Auth: Authorization: Bearer
# <token>.
set -u
...
BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/aos-x-poll.XXXXXX") || exit 0
AUTH_HEADER_FILE=
...
code=$(curl -m 5 -s -o "$BODY_FILE" -w '%{http_code}' \
  ...
  "$AOSX_RELAY/connector/poll" 2>/dev/null) || exit 0
...
# Semantic worthiness is decided by aosx-respond, so acknowledgments can still be
