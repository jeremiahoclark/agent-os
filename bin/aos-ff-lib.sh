...
# Shared fast-forward machinery for maestro self-sync.
# Usage: . bin/aos-ff-lib.sh   (after AOS_ROOT and AOS_HOME are set)
#
# This is the one implementation of "advance a maestro checkout to a base by a
# clean fast-forward, never forcing, merging, or stashing" used by every sync
# path:
#   - /update-agentos (bin/aos-update.sh) pulls from origin: base_mode "origin".
#   - the local-HEAD secondary_subagent sync (bin/aos-spawn.sh on launch, bin/aos-bootstrap.sh
...
#
# Every secondary_subagent home is a worktree of this same repo, so it already holds the
...
# config/, projects/, .no-mistakes/), so a secondary_subagent's backlog, projects, and
...
SUB_HOME_MARKER="${SUB_HOME_MARKER:-.aos_secondary_subagent_home}"
...
# sync target every secondary_subagent follows. Reads the default branch *ref* rather than
...
validate_operational_dirs() {
  ...
  for name in data state config projects; do
    dir="$abs_home/$name"
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      VALIDATION_ERROR="secondary_subagent $name directory must resolve inside the secondary_subagent home"
      return 1
    fi
    if [ -d "$dir" ]; then
      abs_dir=$(cd "$dir" && pwd -P) || {
        VALIDATION_ERROR="secondary_subagent $name directory cannot be resolved"
        return 1
      }
    elif [ -e "$dir" ]; then
      VALIDATION_ERROR="secondary_subagent $name path is not a directory"
      return 1
    else
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      VALIDATION_ERROR="secondary_subagent $name directory cannot be inside the active maestro home"
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      VALIDATION_ERROR="secondary_subagent $name directory cannot be inside the maestro repo"
      return 1
    fi
  done
}
...
validate_secondary_subagent_home() {
  ...
  abs_active_home=$(resolved_existing_dir "$AOS_HOME") || {
    VALIDATION_ERROR="active maestro home is not a directory"
    return 1
  }
  abs_root=$(resolved_existing_dir "$AOS_ROOT") || {
    VALIDATION_ERROR="maestro repo is not a directory"
    return 1
  }
  if [ "$abs_home" = "/" ]; then
    VALIDATION_ERROR="secondary_subagent home cannot be the filesystem root"
    return 1
  fi
  if [ "$abs_home" = "$abs_active_home" ]; then
    VALIDATION_ERROR="secondary_subagent home cannot be the active maestro home"
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    VALIDATION_ERROR="secondary_subagent home cannot be the maestro repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    VALIDATION_ERROR="secondary_subagent home cannot be inside the active maestro home"
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    VALIDATION_ERROR="secondary_subagent home cannot be inside the maestro repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    VALIDATION_ERROR="secondary_subagent home cannot be an ancestor of the active maestro home"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    VALIDATION_ERROR="secondary_subagent home cannot be an ancestor of the maestro repo"
    return 1
  fi
  validate_operational_dirs "$abs_home" "$abs_active_home" "$abs_root" || return 1
  if [ -L "$abs_home/$SUB_HOME_MARKER" ]; then
    VALIDATION_ERROR="secondary_subagent marker must not be a symlink"
    return 1
  fi
  if [ ! -f "$abs_home/$SUB_HOME_MARKER" ]; then
    VALIDATION_ERROR="not a seeded secondary_subagent home"
    return 1
  fi
  marker_id=$(cat "$abs_home/$SUB_HOME_MARKER" 2>/dev/null || true)
  if [ "$marker_id" != "$id" ]; then
    VALIDATION_ERROR="marked for secondary_subagent ${marker_id:-unknown}, expected $id"
    return 1
  fi
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    VALIDATION_ERROR="not a maestro home (missing AGENTS.md)"
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    VALIDATION_ERROR="not a maestro home (missing bin/)"
    return 1
  fi
...
#   origin       - fetch origin and advance to origin/<default> (the /update-agentos
...
#                  dependency (the local-HEAD secondary_subagent sync). The commit must
...
# Validate and fast-forward one secondary_subagent home, accumulating its window into
...
# maestro repo itself (AOS_ROOT) is never processed as its own secondary_subagent, and
# each resolved home is processed at most once.
process_secondary_subagent() {
  local id=$1 home=$2 window=${3:-} base_mode=$4 nudge_requires_instr=${5:-no} home_real fm_root_real
  [ -n "$id" ] || return 0
  [ -n "$home" ] || return 0
  fm_root_real=$(resolve_path "$AOS_ROOT")
  home_real=$(resolve_path "$home")
  [ "$home_real" != "$fm_root_real" ] || return 0
  if ! validate_secondary_subagent_home "$id" "$home"; then
    echo "secondary_subagent $id: skipped: unsafe home: $VALIDATION_ERROR"
    return 0
  fi
  ...
  ff_target "$home_real" "secondary_subagent $id" "$base_mode" yes yes
...
# Sweep this home's LIVE secondary_subagent direct reports - state/<id>.meta files with
# kind=secondary_subagent - fast-forwarding each to base_mode. Passes base_mode and
# nudge_requires_instr through to process_secondary_subagent. Accumulates into
# FF_NUDGE_WINDOWS / FF_SEEN_HOMES, which the caller resets before and reads after.
sweep_live_secondary_subagent_metas() {
  ...
  for meta in "$state"/*.meta; do
    ...
    process_secondary_subagent "$id" "$home" "$window" "$base_mode" "$nudge_requires_instr"
  done
}
