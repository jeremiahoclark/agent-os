#!/usr/bin/env bash
# Safe, home-scoped (re-)arm of the maestro watcher, with honest verification.
#
# The watcher (bin/aos-watch.sh) blocks until it has an actionable wake to
...
# call and NOTIFIES on exit, so maestro must run this script as the harness's
...
# liveness beacon (state/.last-watcher-beat) is fresh within AOS_GUARD_GRACE (the
# single source of truth, shared with aos-watch.sh and aos-guard.sh), and prints
...
#
# --restart: stop ONLY this AOS_HOME's watcher (the pid recorded in THIS home's
...
# bin/aos-watch.sh`: that pattern matches every maestro home's watcher
# (secondary_subagent homes run the same script) and would kill siblings.
set -u
...
# wake exit propagates out so the harness re-notifies maestro.
child=
child_out=
cleanup_child() {
