...
# token-tight line maestro can read every heartbeat:
#
...
ID=${1:-}
[ -n "$ID" ] || { echo "usage: aos-subagent-state.sh <id>" >&2; exit 2; }

META="$STATE/$ID.meta"
LOG="$STATE/$ID.status"
NM_TIMEOUT=${AOS_SUBAGENT_STATE_NM_TIMEOUT:-10}
...
HAVE_RUN=0
# Scouts and secondary_subagents never drive a no-mistakes validation of their own
...
if [ "$HAVE_RUN" = 1 ]; then
  ...
  if [ -n "$outcome" ]; then
    ...
    if printf '%s\n' "$RUN_OUT" | grep -q 'ask-user'; then
      RUN_DETAIL="$RUN_DETAIL (ask-user: owner decision)"
    fi
  else
    case "$status" in
...
# Secondary_subagents idle on their own watcher (idle pane = healthy), so the busy
# signature is not meaningful for them; read their state from the status log only.
if [ "$KIND" != secondary_subagent ] && fm_pane_is_busy "$WIN"; then
