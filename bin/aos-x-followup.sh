...
#
# An X mention that spawned real work is linked to its task by aos-x-link.sh
# (x_request/x_request_ts in state/<id>.meta). When that task reaches a terminal
# state (PR merged / scout report / local merge / failed), maestro composes a
...
#   aos-x-followup.sh --check <task-id>
...
#   aos-x-followup.sh <task-id> --text-file <path>
#   aos-x-followup.sh <task-id> -
#     Linked and within window: posts ONE follow-up via aos-x-reply.sh
...
#
# Dry-run (AOSX_DRY_RUN) flows through aos-x-reply.sh: the follow-up is recorded to
...
#
# The 24h window is AOSX_FOLLOWUP_MAX_AGE_SECS (default 86400). AOSX_NOW_OVERRIDE
# pins "now" for deterministic tests. Meta read/write lives in aos-x-lib.sh.
set -u
...
usage() {
  echo "usage: aos-x-followup.sh --check <task-id> | <task-id> --text-file <path> | <task-id> -" >&2
}

MAX_AGE=${AOSX_FOLLOWUP_MAX_AGE_SECS:-86400}
case "$MAX_AGE" in
  ''|*[!0-9]*) MAX_AGE=86400 ;;
esac
...
case "$ID" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "aos-x-followup: unsafe task id: $ID" >&2; exit 2 ;;
esac
...
# a post is simply a no-op success (maestro need not special-case it).
if [ -z "$RID" ]; then
  if [ "$MODE" = check ]; then
    exit 1
  fi
  echo "aos-x-followup: $ID is not X-linked; nothing to post" >&2
  exit 0
fi

NOW=${AOSX_NOW_OVERRIDE:-$(date +%s)}
case "$NOW" in
  ''|*[!0-9]*) echo "aos-x-followup: could not read the current time" >&2; exit 1 ;;
esac
...
if [ "$EXPIRED" = 1 ]; then
  fmx_meta_link_clear "$META" || echo "aos-x-followup: warning: could not clear the elapsed link in state/$ID.meta" >&2
  if [ "$MODE" = check ]; then
    exit 1
  fi
  echo "aos-x-followup: follow-up window elapsed for $ID; skipped and cleared the link" >&2
  exit 0
fi
...
# Post the follow-up. aos-x-reply owns text reading, thread-split, dry-run, the
# endpoint, and the never-inline safety; we only pass the text source through.
if "$AOS_ROOT/bin/aos-x-reply.sh" "$RID" --followup "${TS_ARGS[@]}" >/dev/null; then
  fmx_meta_link_clear "$META" || echo "aos-x-followup: warning: posted but could not clear the link in state/$ID.meta" >&2
  printf '%s\n' "$RID"
  exit 0
fi

# Post failed: leave the link so maestro can retry on a later pass.
echo "aos-x-followup: follow-up post failed for $ID; left the link in place to retry" >&2
exit 1
