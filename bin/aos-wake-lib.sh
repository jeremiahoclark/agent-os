...
AOS_WAKE_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
AOS_WAKE_DEFAULT_ROOT="$(cd "$AOS_WAKE_LIB_DIR/.." && pwd)"
AOS_ROOT="${AOS_ROOT_OVERRIDE:-${AOS_ROOT:-$AOS_WAKE_DEFAULT_ROOT}}"
AOS_HOME="${AOS_HOME:-${AOS_ROOT_OVERRIDE:-$AOS_ROOT}}"
STATE="${AOS_STATE_OVERRIDE:-${STATE:-$AOS_HOME/state}}"
AOS_WAKE_QUEUE="${AOS_WAKE_QUEUE:-$STATE/.wake-queue}"
AOS_WAKE_QUEUE_LOCK="${AOS_WAKE_QUEUE_LOCK:-$STATE/.wake-queue.lock}"
AOS_LOCK_STALE_AFTER="${AOS_LOCK_STALE_AFTER:-2}"
mkdir -p "$STATE"
...
fm_lock_mid_acquire_is_fresh() {
  ...
  case "$pid" in
    ''|*[!0-9]*)
      mid_acquire_stale=$AOS_LOCK_STALE_AFTER
...
fm_lock_try_acquire() {
  ...
  if ! fm_lock_try_acquire "$steal"; then
    AOS_LOCK_HELD_PID=$(cat "$lockdir/pid" 2>/dev/null || true)
    AOS_LOCK_OWNER_DIR=
    return 1
  fi
...
fm_wake_append() {
  ...
  printf '%s\n' "$seq" > "$seq_file" || status=$?
  if [ "$status" -eq 0 ]; then
    printf '%s\t%s\t%s\t%s\t%s\n' "$epoch" "$seq" "$kind" "$clean_key" "$clean_payload" >> "$AOS_WAKE_QUEUE" || status=$?
  fi
...
fm_wake_restore_queue() {
  ...
  if [ -e "$AOS_WAKE_QUEUE" ]; then
    cat "$drained" "$AOS_WAKE_QUEUE" > "$restore" && mv "$restore" "$AOS_WAKE_QUEUE"
  else
    mv "$drained" "$AOS_WAKE_QUEUE"
  fi
}
