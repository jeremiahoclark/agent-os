...
tradeoff **consented** and **explicit**: the owner is stepping away, so the
sub-supervisor may triage routine wakes in bash instead of waking maestro's
LLM for each one. Escalations still reach the owner, but as one pre-read,
...
1. **Set the durable away-mode flag:**
   ...
   date '+%s' > state/.afk
   ```
   This file survives a maestro restart: recovery re-enters afk if the
   flag is present.
...
2. **Ensure the sub-supervisor daemon is running.** Check the pid file; start
   ...
   if [ -f state/.supervise-daemon.pid ] && kill -0 "$(cat state/.supervise-daemon.pid)" 2>/dev/null; then
     ...
     nohup bin/aos-supervise-daemon.sh >/dev/null 2>&1 &
   fi
   ```
   The daemon is **presence-gated**: it injects escalations only while
   `state/.afk` exists, and stays quiet otherwise.

3. **Do not separately arm `aos-watch.sh`.** The daemon manages the watcher as
   its child; the singleton lock no-ops a stray arm harmlessly.

4. **Acknowledge** to the owner that away-mode is active: the daemon will
   self-handle routine wakes, escalate only owner-relevant events, and the
   owner can exit by sending any real message.
...
- A message **without** the sentinel marker and **not** starting with `/afk`
  -> the owner is back. Clear `state/.afk`, stop the daemon, flush one
  ...
  responsiveness (arm `bin/aos-watch-arm.sh`).
- A message **with** the sentinel marker (`AOS_INJECT_MARK`, ASCII 0x1f) -> it
...
Bias ambiguous cases toward exit: a present owner beats token savings, and
a false exit is self-correcting (the owner re-runs `/afk`).

## Orthogonal to approval authority

afk changes how aggressively maestro surfaces things, **not who approves
what**. "Away" never means "approves more." A PR ready for merge, a
needs-decision finding, or anything destructive still waits for the owner's
explicit word - the daemon just batches the notification.

## Sentinel marker contract

The daemon prefixes every injection with `AOS_INJECT_MARK` (ASCII unit
separator, 0x1f), invisible and untypable. This is how maestro tells a
...
injection (shared with `aos-send.sh` via `bin/aos-tmux-lib.sh`):
...
- **`pane_input_pending`** - the cursor line holds real unsubmitted text (a
  ...
  *bordered* composer (claude draws `│ > … │`) is correctly read as empty, not
  ...
  `AOS_COMPOSER_IDLE_RE` still overrides empty-composer matching after border
  stripping.
...
protects against the race window between the owner returning and their
message landing, and against the daemon's own previous injection sitting unsent.

**Max-defer escape (the daemon must never silently wedge).**
If anything stays buffered past `AOS_MAX_DEFER_SECS` (default 300), the daemon
...
`aos-send.sh` uses the same primitive and exits non-zero
when a steer's Enter is positively swallowed, so maestro learns an instruction
did not land instead of leaving it unsubmitted.

## Classification policy

The daemon wraps `aos-watch.sh`, runs the watcher as a child, classifies each
wake reason in bash, and self-handles the routine majority without consuming a
maestro turn.
Only owner-relevant events escalate to maestro's context, and even then as
one pre-read, single-line, batched digest.
The classification predicates (the owner-relevant verb set, the signal/stale
tests, and the fleet-scan) live in the shared `bin/aos-classify-lib.sh`, the same
...
- `signal` whose status content has no owner-relevant verb
  (`done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged`)
  -> self-handle. Owner-relevant verb -> escalate.
- `check` -> always escalate. Check scripts print only when maestro should wake.
- `stale` with a terminal status -> escalate. Non-terminal stale is transient:
  ...
  `AOS_STALE_ESCALATE_SECS` (default 240s), housekeeping escalates it as a
  possible wedge. This bounds wedge-detection latency to the threshold plus a
  tick: a delay, never a loss. Healthy subagents are autonomous and do not wait
  on maestro mid-task.
- `heartbeat` -> self-handle. The daemon runs its own cheap bash fleet scan
  every `AOS_HEARTBEAT_SCAN_SECS` (default 300s) as the catch-all for a
  owner-relevant status line the per-wake classifier might miss.
- Unknown reason, or any uncertainty -> escalate fail-safe.

Escalations are buffered up to `AOS_ESCALATE_BATCH_SECS` (default 90s; 0 =
...
the marker lets maestro distinguish it from a real owner message.
...
- **Composer guard on the supervisor pane** - before injecting, the daemon
  ...
  its digest into the owner's half-typed line.
- The composer detector, shared with `aos-send.sh` in `bin/aos-tmux-lib.sh`, drops
  ...
  suggestions can look like pending input and stall supervision. `AOS_COMPOSER_IDLE_RE`
  still overrides empty-composer matching after dim-ghost and border stripping,
  and `AOS_BUSY_REGEX` overrides busy footers.
- **Max-defer escape** - the daemon must never silently wedge. If anything stays
  buffered past `AOS_MAX_DEFER_SECS` (default 300s), the daemon attempts one
...
- **Marker strip** - `strip_injection_marker` removes the sentinel prefix before
  classification or relay, so the digest text maestro sees is clean.
- **Portable singleton lock** - the daemon uses the repo's portable lock helper
  (`aos-wake-lib.sh`) instead of `flock`, which is absent on macOS.
...
- **Auto-discovered supervisor pane** - the daemon resolves its injection target
  from `AOS_SUPERVISOR_TARGET`, then `$TMUX_PANE`, then a `maestro:0` fallback
...
- Nothing is lost. The durable queue plus `aos-wake-drain.sh` recover any missed
...
`AOS_INJECT_SKIP` (default `heartbeat`) force-self-handles matching kinds,
