...
calendar invites, threads the owner has replied to, senders marked
...
1. `bliss bulk-archive` (dry-run). Report only the top-line numbers to the
   owner: total eligible, number of sender groups, top 5 senders by count.
...
3. Relay mimo's report. If the owner says some sender should never be
...
4. If we're being too aggressive (owner flags a miss), note the sender as
   never-archive and mention the miss in the daily note's review section so
   the pattern is tracked. The owner has accepted aggressive-for-now.
